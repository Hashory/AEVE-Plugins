...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_BLEND, STR_ON, FALSE, 0, ID_BLEND);
  //----------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
static PF_Err RedBlendIn8(refconType refcon, A_long xL, A_long yL,
...
//-------------------------------------------------------------------------------------------------
static PF_Err RedBlendIn16(refconType refcon, A_long xL, A_long yL,
...
//-------------------------------------------------------------------------------------------------
static PF_Err RedBlendIn32(refconType refcon, A_long xL, A_long yL,
...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  PF_FpLong ds = (PF_FpLong)in_data->downsample_x.num /
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  for (A_long dc = 0; dc < infoP->drawCount; dc++) {
    for (A_long pc = 0; pc < infoP->point_count; pc++) {
      PointInfo p =
          ld.Points.EnFromRot(infoP->center, infoP->point_rot + dr * pc,
                              infoP->radius, infoP->aspect, infoP->rot);
    ...
    for (A_long pc = 0; pc < infoP->point_count; pc++) {
      ...
      if (infoP->foldCount > 0) {
        for (A_long fc = 0; fc < infoP->foldCount; fc++) {
          ld.Points.CalcCenterPos(infoP->lineSize, infoP->lineMove,
                                  infoP->offset, depth);
          depth++;
        }
      }
      ...
      if (infoP->subCount > 0) {
        for (A_long sc = 0; sc < infoP->subCount; sc++) {
          ...
          if (infoP->foldCount > 0) {
            for (A_long fc = 0; fc < infoP->foldCount; fc++) {
              ld.Points.CalcCenterPos(infoP->lineSize / 3,
                                      infoP->lineMove * 5 / 4, -infoP->offset,
                                      depth);
              depth++;
            }
          }
