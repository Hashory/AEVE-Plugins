...
static PF_Err About(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_SPRINTF(out_data->return_msg, "%s, v%d.%d%d\r%s", STR(StrID_Name),
             MAJOR_VERSION, MINOR_VERSION, BUG_VERSION, STR(StrID_Description));
  return PF_Err_NONE;
}
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR(StrID_MY_Main_Color), 0x00, 0x00, 0x00, MY_Main_Color);
...
PF_Err CMainLineRepaint::getParams(PF_InData *in_data, PF_OutData *out_data,
                                   PF_ParamDef *params[], PF_LayerDef *output) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err Render(PF_InData *in_data, PF_OutData *out_data,
  ...
  if (exe.Enabled() == FALSE) {
    PF_SPRINTF(out_data->return_msg, "[%s] %s", STR(StrID_Name),
