...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_Highlights_COLOR, Highlights_def_red, Highlights_def_green,
               Highlights_def_blue, ID_Highlights_COLOR);
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_Midtones_COLOR, Midtones_def_red, Midtones_def_green,
               Midtones_def_blue, ID_Midtones_COLOR);
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_Shadows_COLOR, Shadows_def_red, Shadows_def_green,
               Shadows_def_blue, ID_Shadows_COLOR);
  // Slider for fixed-point numbers
  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(STR_Midtones_OFFSET,  // Parameter name
               -100,                 // Minimum value for numeric input
               100,                  // Maximum value for numeric input
               -50,                  // Minimum value for slider
               50,                   // Maximum value for slider
               Midtones_def_offset,  // Default value
               1,                    // Flags for numeric display
               0, 0, ID_Midtones_OFFSET);
  // Slider for fixed-point numbers
  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(STR_Blend_w_Original_FIXED,  // Parameter name
               0,                           // Minimum value for numeric input
               100,                         // Maximum value for numeric input
               0,                           // Minimum value for slider
               100,                         // Maximum value for slider
               Blend_w_Original_def,        // Default value
               1,                           // Flags for numeric display
               0, 0, ID_Blend_w_Original_FIXED);
...
static PF_Err QueryDynamicFlags(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_Err err = PF_Err_NONE, err2 = PF_Err_NONE;
  // Set the PF_OutFlag_NON_PARAM_VARY value to out_flags to switch
...
static PF_PixelFloat colcColor(ToneParam *prm) {
  ...
  if (l <= 0) {
    ...
    if (l < b) {
      v = l / b;
      ret.blue =
          prm->Shadows.blue + (prm->Midtones.blue - prm->Shadows.blue) * v;
      ret.green =
          prm->Shadows.green + (prm->Midtones.green - prm->Shadows.green) * v;
      ret.red = prm->Shadows.red + (prm->Midtones.red - prm->Shadows.red) * v;
    } else {
      v = (PF_FpShort)((l - b) / (1.0 - b));
      ret.blue = (PF_FpShort)(prm->Midtones.blue +
                              (prm->Highlights.blue - prm->Midtones.blue) * v);
      ret.green =
          (PF_FpShort)(prm->Midtones.green +
                       (prm->Highlights.green - prm->Midtones.green) * v);
      ret.red = (PF_FpShort)(prm->Midtones.red +
                             (prm->Highlights.red - prm->Midtones.red) * v);
    }
  }
  return ret;
}
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (!tbl) {
    ...
    A_long v =
        ((outP->red * 77) + (outP->green * 151) + (outP->blue * 28)) >> 8;
    if (v < 0) v = 0;
    ...
    if (niP->Blend_w_Original == 0) {
      ...
      outP->red =
          RoundByteFpLong(((PF_FpShort)outP->red * niP->Blend_w_Original) +
                          ((PF_FpShort)col.red * s));
      outP->green =
          RoundByteFpLong(((PF_FpShort)outP->green * niP->Blend_w_Original) +
                          ((PF_FpShort)col.green * s));
      outP->blue =
          RoundByteFpLong(((PF_FpShort)outP->blue * niP->Blend_w_Original) +
                          ((PF_FpShort)col.blue * s));
    }
  }
  return err;
}
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  if (!tbl) {
    ...
    PF_FpShort v = (((PF_FpShort)outP->red * 77 / 256) +
                    ((PF_FpShort)outP->green * 151 / 256) +
                    ((PF_FpShort)outP->blue * 28 / 256));
    if (v < 0) v = 0;
    ...
    if (niP->Blend_w_Original == 0) {
      ...
      PF_FpShort s = (PF_FpShort)1.0 - niP->Blend_w_Original;
      outP->red =
          RoundShortFpLong(((PF_FpShort)outP->red * niP->Blend_w_Original) +
                           ((PF_FpShort)col.red * s));
      outP->green =
          RoundShortFpLong(((PF_FpShort)outP->green * niP->Blend_w_Original) +
                           ((PF_FpShort)col.green * s));
      outP->blue =
          RoundShortFpLong(((PF_FpShort)outP->blue * niP->Blend_w_Original) +
                           ((PF_FpShort)col.blue * s));
    }
  }
  return err;
...
static PF_Err FilterImage32(refconType refcon, A_long xL, A_long yL,
  ...
  g = (PF_FpShort)((outP->red * 77 / 256) + (outP->green * 151 / 256) +
                   (outP->blue * 28 / 256));
  if (g < 0)
    g = 0;
  else if (g > 1.0)
  ...
  if (niP->Blend_w_Original == 0) {
    ...
    PF_FpShort s = (PF_FpShort)1.0 - niP->Blend_w_Original;
    outP->red =
        RoundFpShort((outP->red * niP->Blend_w_Original) + (col.red * s));
    outP->green =
        RoundFpShort((outP->green * niP->Blend_w_Original) + (col.green * s));
    outP->blue =
        RoundFpShort((outP->blue * niP->Blend_w_Original) + (col.blue * s));
  }

  return err;
}
static PF_Err GetParams(CFsAE *ae, ToneParam *infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err Exec(CFsAE *ae, ToneParam *tp) {
  PF_Err err = PF_Err_NONE;
