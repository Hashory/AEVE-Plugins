...
static PF_Err AlphaFill8(void* refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                         PF_Pixel8* outP) {
  ...
  outP->blue = RoundByteDouble((double)infoP->dst8.blue * (1 - op) +
                               (double)inP->blue * op + 0.5);
  outP->green = RoundByteDouble((double)infoP->dst8.green * (1 - op) +
                                (double)inP->green * op + 0.5);
  outP->red = RoundByteDouble((double)infoP->dst8.red * (1 - op) +
                              (double)inP->red * op + 0.5);

  return err;
}
static PF_Err AlphaFill16(void* refcon, A_long xL, A_long yL, PF_Pixel16* inP,
                          PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
  ...
  outP->blue = RoundShortFpLong((double)infoP->dst16.blue * (1 - op) +
                                (double)inP->blue * op + 0.5);
  outP->green = RoundShortFpLong((double)infoP->dst16.green * (1 - op) +
                                 (double)inP->green * op + 0.5);
  outP->red = RoundShortFpLong((double)infoP->dst16.red * (1 - op) +
                               (double)inP->red * op + 0.5);

  return err;
}
static PF_Err AlphaFill32(void* refcon, A_long xL, A_long yL,
  ...
  outP->blue = RoundFpShortDouble((double)infoP->dst32.blue * (1 - op) +
                                  (double)inP->blue * op);
  outP->green = RoundFpShortDouble((double)infoP->dst32.green * (1 - op) +
                                   (double)inP->green * op);
  outP->red = RoundFpShortDouble((double)infoP->dst32.red * (1 - op) +
                                 (double)inP->red * op);

  return err;
}
static PF_Err ColorChange8(void* refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                           PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);

  PF_Pixel t = *inP;
  if ((t.red == infoP->src.red) && (t.green == infoP->src.green) &&
...
static PF_Err ColorChange16(void* refcon, A_long xL, A_long yL, PF_Pixel16* inP,
                            PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
static PF_Err ColorChange32(void* refcon, A_long xL, A_long yL,
                            PF_PixelFloat* inP, PF_PixelFloat* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
PF_Err ColorChangeFromPoint::ParamsSetup(PF_InData* in_dataP,
  ...
  // Specify the color
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_CANNOT_TIME_VARY;  // If this is enabled, keyframes
                                              // cannot be set
  PF_ADD_COLOR(STR_COLOR, 0xFF, 0x00, 0x00, ID_COLOR);
...
PF_Err GetTargetColor(NFsWorld* w, ParamInfo* infoP) {
  ...
  infoP->mode = MODE_NONE;
  // Area Over
  if ((infoP->pointX < 0) || (infoP->pointY < 0) ||
      (infoP->pointX >= w->width()) || (infoP->pointY >= w->height())) {
    return err;
  }
