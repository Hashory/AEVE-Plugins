...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_REV1, STR_REV2, FALSE, 0, ID_CB);
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    A_long v = ((inP->red * 77) + (inP->green * 151) + (inP->blue * 28)) *
               inP->alpha / (256 * PF_MAX_CHAN8);
...
static PF_Err FilterImage32(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    PF_FpShort v = ((inP->red * 77 / 256) + (inP->green * 151 / 256) +
                    (inP->blue * 28 / 256)) *
                   inP->alpha;
    outP->blue = niP->col.blue;
    outP->green = niP->col.green;
