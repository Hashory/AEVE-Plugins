  ...
  try {
    ...
    if ((!err) && (in_data->appl_id != 'PrMr')) {
      ...
      if (!err) {
        ...
        for (A_long i = 0; i < PixelReplace_NUM_PARAMS; i++)
          hide_themB[i] = TRUE;
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  for (i = 0; i < PR_PRM_COUNT; i++) {
    ...
    //-----------------
    AEFX_CLR_STRUCT(def);
    PF_ADD_PERCENT(UI_REP_OPACITY, 100, PARAMS_IDX(i, paramsOffset_d_opacity));
    //-----------------
    AEFX_CLR_STRUCT(def);
  ...
  for (A_long i = 0; i < infoP->dispCount; i++) {
    ...
    if ((!err) && (flg == TRUE)) {
      ERR(ae->GetCOLOR(PARAMS_IDX(i, paramsOffset_s_color),
                       &infoP->src[count]));
      ERR(ae->GetCOLOR(PARAMS_IDX(i, paramsOffset_d_color),
                       &infoP->dst[count]));
      PF_Fixed f;
