...
static PF_Err NFsNoise8(void *refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                        PF_Pixel8 *outP) {
...
static PF_Err NFsNoise16(void *refcon, A_long xL, A_long yL, PF_Pixel16 *inP,
                         PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *infoP = reinterpret_cast<ParamInfo *>(refcon);
...
static PF_Err NFsNoise32(void *refcon, A_long xL, A_long yL, PF_PixelFloat *inP,
                         PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *infoP = reinterpret_cast<ParamInfo *>(refcon);
...
PF_Err UsedColorList::ParamsSetup(PF_InData *in_dataP, PF_OutData *out_dataP,
                                  PF_ParamDef *paramsP[],
                                  PF_LayerDef *outputP) {
  PF_Err err = PF_Err_NONE;
  Init();
  ...
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_ExceptColor0, 0xFF, 0x0, 0x0, ID_ExceptColor0);
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_ExceptColor1, 0x00, 0xFF, 0x0, ID_ExceptColor1);
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_ExceptColor2, 0x00, 0x00, 0xFF, ID_ExceptColor2);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_ISDISP, STR_ON, TRUE, 0, ID_ISDISP);
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER(STR_GRID_WIDTH,  // Parameter name
                10,              // Minimum value for numerical input
                300,             // Maximum value for numerical input
                10,              // Minimum value of the slider
                100,             // Maximum value of the slider
                50,              // Default value
                ID_GRID_WIDTH);
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER(STR_GRID_HEIGHT,  // Parameter name
                10,               // Minimum value for numerical input
                300,              // Maximum value for numerical input
                10,               // Minimum value of the slider
                100,              // Maximum value of the slider
