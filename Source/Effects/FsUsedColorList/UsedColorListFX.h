...
class UsedColorListFX : public NFsWorld {
 private:
 public:
  UsedColorListFX(PF_EffectWorld* wld = NULL, PF_InData* ind = NULL,
  ...
  void DrawGrid(PF_Pixel col, A_Rect rct) {
    ...
    for (A_long j = rct.top; j < rct.bottom; j++)
      for (A_long i = rct.left; i < rct.right; i++) SetPix8(i, j, col);
  }
  ...
  A_long IndexOfColor(PF_Pixel* tbl, A_long tblMax, PF_Pixel c) {
    ...
    for (A_long i = 0; i < tblMax; i++) {
      if ((tbl[i].red == c.red) && (tbl[i].green == c.green) &&
          (tbl[i].blue == c.blue)) {
        ret = i;
        break;
      }
  ...
  void bubbleSort(PF_Pixel* p, A_long* s, A_long array_num) {
    for (A_long i = 0; i < array_num - 1; i++) {
      // Sort from the right (sorting from the left would require one more
      // variable)
      for (A_long j = array_num - 1; i < j; j--) {
        if (s[j] < s[j - 1]) {  // If the left number is larger, swap
          // Swap
          PF_Pixel tc = p[j];
          p[j] = p[j - 1];
          p[j - 1] = tc;
