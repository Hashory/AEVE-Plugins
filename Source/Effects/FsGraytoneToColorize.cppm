...
static PF_Err (*func8)(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                       PF_Pixel8 *outP);

static PF_Err (*func16)(refconType refcon, A_long xL, A_long yL,
                        PF_Pixel16 *inP, PF_Pixel16 *outP);

static PF_Err (*func32)(refconType refcon, A_long xL, A_long yL,
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_OFFSET_ON, "on", TRUE, 0, ID_OFFSET_ON);
  //----------------------------------------------------------------
  // 角度
  AEFX_CLR_STRUCT(def);
...
//-------------------------------------------------------------------------------------------------
static PF_Err execTargetV8(refconType refcon, A_long xL, A_long yL,
  ...
  switch (infoP->target) {
    case 2:
      outP->blue = outP->green = outP->red =
          RoundByteLong((A_long)((double)inP->red * a + 0.5));
      break;
    case 3:
      outP->blue = outP->green = outP->red =
          RoundByteLong((A_long)((double)inP->green * a + 0.5));
      break;
    case 4:
      outP->blue = outP->green = outP->red =
          RoundByteLong((A_long)((double)inP->blue * a + 0.5));
      break;
    case 5:
    ...
    case 1:
      ...
      A_long v = (A_long)((0.29891 * (double)r) + (0.58661 * (double)g) +
                          (0.11448 * (double)b) + 0.5);
...
//-------------------------------------------------------------------------------------------------
static PF_Err execTargetV16(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *infoP = reinterpret_cast<ParamInfo *>(refcon);
  ...
  switch (infoP->target) {
    case 2:
      outP->blue = outP->green = outP->red =
          RoundShort((A_long)((double)inP->red * a + 0.5));
      break;
    case 3:
      outP->blue = outP->green = outP->red =
          RoundShort((A_long)((double)inP->green * a + 0.5));
      break;
    case 4:
      outP->blue = outP->green = outP->red =
          RoundShort((A_long)((double)inP->blue * a + 0.5));
      break;
    case 5:
      outP->blue = outP->green = outP->red = inP->alpha;
      break;
...
//-------------------------------------------------------------------------------------------------
static PF_Err execTargetV32(refconType refcon, A_long xL, A_long yL,
                            PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *infoP = reinterpret_cast<ParamInfo *>(refcon);
...
inline PF_Pixel colBlend8(PF_Pixel s, PF_Pixel d, double p) {
  ...
  if (p == 0)
    ...
    return d;
  ret.red =
      RoundByteLong((A_long)((double)d.red * p + (double)s.red * (1 - p)));
  ret.green =
      RoundByteLong((A_long)((double)d.green * p + (double)s.green * (1 - p)));
  ret.blue =
      RoundByteLong((A_long)((double)d.blue * p + (double)s.blue * (1 - p)));
  return ret;
}
//-------------------------------------------------------------------------------------------------
inline PF_Pixel16 colBlend16(PF_Pixel16 s, PF_Pixel16 d, double p) {
  ...
  if (p == 0)
    ...
    return d;
  ret.red =
      RoundShortFpLong((A_long)((double)d.red * p + (double)s.red * (1 - p)));
  ret.green = RoundShortFpLong(
      (A_long)((double)d.green * p + (double)s.green * (1 - p)));
  ret.blue =
      RoundShortFpLong((A_long)((double)d.blue * p + (double)s.blue * (1 - p)));
  return ret;
}
//-------------------------------------------------------------------------------------------------
inline PF_PixelFloat colBlend32(PF_PixelFloat s, PF_PixelFloat d, double p) {
  ...
  ret.green =
      RoundFpShortDouble(((double)d.green * p + (double)s.green * (1 - p)));
  ret.blue =
      RoundFpShortDouble(((double)d.blue * p + (double)s.blue * (1 - p)));
  return ret;
}
//-------------------------------------------------------------------------------------------------
static PF_Err execColorize8(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *infoP = reinterpret_cast<ParamInfo *>(refcon);
  ...
  if (idx == 0) {
    *outP = infoP->col[0];
    ...
    double p = ((double)v - (double)infoP->colBorder[idx - 1]) /
               (double)(infoP->colBorder[idx] - infoP->colBorder[idx - 1]);
...
//-------------------------------------------------------------------------------------------------
static PF_Err execColorize16(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
//-------------------------------------------------------------------------------------------------
static PF_Err execColorize32(refconType refcon, A_long xL, A_long yL,
                             PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
