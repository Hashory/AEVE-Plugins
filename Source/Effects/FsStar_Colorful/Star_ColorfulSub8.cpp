...
inline PF_Pixel GetPX(GInfo* gi, A_long x, A_long y) {
  ...
  if (yy < 0)
    yy = 0;
  else if (yy >= gi->height)
    xx = gi->height - 1;
  return data[xx + yy * gi->widthTrue];
}

// Basic part of screen synthesis
...
// Screen composition that supports decimal points
...
static PF_Err BlendScreen(refconType refcon, A_long xL, A_long yL,
...
static PF_Err BlendAdd(refconType refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                       PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ...
  outP->red =
      RoundByteLong((A_long)outP->red + (A_long)((PF_FpLong)inP->red * a));
  outP->green =
      RoundByteLong((A_long)outP->green + (A_long)((PF_FpLong)inP->green * a));
  outP->blue =
      RoundByteLong((A_long)outP->blue + (A_long)((PF_FpLong)inP->blue * a));

  return err;
}
static PF_Err BlendNormal(refconType refcon, A_long xL, A_long yL,
                          PF_Pixel8* inP, PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err TargetPixel(refconType refcon, A_long xL, A_long yL,
                          PF_Pixel8* inP, PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ...
  switch (infoP->info.target_kind) {
    ...
    case 4:
      ...
      v = 1.0 - (PF_SQRT(PF_POW(lab.L - infoP->taget_lab.L, 2) +
...
static PF_Err CopyAtoTemp(ParamInfo8* infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err TempRev(ParamInfo8* infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err TempSub(ParamInfo8* infoP) {
  PF_Err err = PF_Err_NONE;
  GInfo gi = infoP->gtmp;
...
static PF_Err MaxHor(ParamInfo8* pi) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err MaxVer(ParamInfo8* pi) {
  PF_Err err = PF_Err_NONE;
...
static void MakePixelTable(PF_Pixel* tbl, A_long len, PF_Pixel col,
                           PF_FpLong per, PF_FpLong br) {
  if ((len <= 0) || (per <= 0)) {
    return;
  }
  ...
  for (A_long i = 0; i < len2; i++) {
    ...
    tbl[i].red = RoundByteLong(tbl[i].red +
                               (A_long)((PF_FpLong)r * br * (len2 - i) / len2));
    tbl[i].green = RoundByteLong(
        tbl[i].green + (A_long)((PF_FpLong)g * br * (len2 - i) / len2));
    tbl[i].blue = RoundByteLong(
        tbl[i].blue + (A_long)((PF_FpLong)b * br * (len2 - i) / len2));
  }
}
static PF_Err StarDrawColor(ParamInfo8* pi) {
  ...
  MakePixelTable(pi->horTable, pi->hor.len, pi->color, pi->hor.opa,
                 pi->info.brigthness);
  MakePixelTable(pi->verTable, pi->ver.len, pi->color, pi->ver.opa,
                 pi->info.brigthness);
  MakePixelTable(pi->diaTable, pi->dia.len, pi->color, pi->dia.opa,
                 pi->info.brigthness);
...
static PF_Err CenterDraw(ParamInfo8* pi) {
  ...
  for (A_long y = 0; y < go.height; y++) {
    for (A_long x = 0; x < go.width; x++) {
      ...
      dataO[posO].alpha =
          RoundByteLong((A_long)dataO[posO].alpha + (A_long)dataT[posT].green);
  ...
  for (A_long y = 0; y < go.height; y++) {
    for (A_long x = 0; x < go.width; x++) {
      ...
      if ((pi->ver.len > 0) && (pi->ver.opa > 0)) {
        A_long vl = pi->ver.len * lv / PF_MAX_CHAN8;
        if (vl > 0) {
          MakePixelTable(pi->verTable, pi->ver.len, c, pi->ver.opa * lvb,
                         pi->info.brigthness);
          A_long d = pi->ver.len - vl;
          go.xD = (PF_FpLong)x;
          go.yD = (PF_FpLong)y;
      ...
      if ((pi->hor.len > 0) && (pi->hor.opa > 0)) {
        A_long hl = pi->hor.len * lv / PF_MAX_CHAN8;
        if (hl > 0) {
          MakePixelTable(pi->horTable, pi->hor.len, c, pi->hor.opa * lvb,
                         pi->info.brigthness);
          A_long d = pi->hor.len - hl;
          go.xD = (PF_FpLong)x;
          go.yD = (PF_FpLong)y;
      ...
      if ((pi->dia.len > 0) && (pi->dia.opa > 0)) {
        A_long dl = pi->dia.len * lv / PF_MAX_CHAN8;
        if (dl > 0) {
          MakePixelTable(pi->diaTable, pi->dia.len, c, pi->dia.opa * lvb,
                         pi->info.brigthness);
          A_long d = pi->dia.len - dl;
          go.xD = (PF_FpLong)x;
          go.yD = (PF_FpLong)y;
...
static void MakeRainbowTable(PF_Pixel* tbl, A_long len, PF_FpLong ro,
                             PF_FpLong rd, PF_FpLong per, PF_FpLong br) {
  if ((len <= 0) || (per <= 0)) {
    return;
  }
...
static PF_Err StarDrawRainbow(ParamInfo8* pi) {
  ...
  MakeRainbowTable(pi->verTable, pi->ver.len, pi->info.rainbowOffset,
                   pi->info.rainbowDelta, pi->ver.opa, pi->info.brigthness);
  MakeRainbowTable(pi->horTable, pi->hor.len, pi->info.rainbowOffset,
                   pi->info.rainbowDelta, pi->hor.opa, pi->info.brigthness);
  MakeRainbowTable(pi->diaTable, pi->dia.len, pi->info.rainbowOffset,
                   pi->info.rainbowDelta, pi->dia.opa, pi->info.brigthness);
...
static PF_Err ToParam8(CFsAE* ae, ParamInfo* infoP, ParamInfo8* pi) {
  ...
  if ((pi->info.autoRolling == TRUE) && (pi->info.rollingSpeed != 0)) {
    double time =
        ((double)ae->in_data->current_time / (double)ae->in_data->time_scale);
...
PF_Err StarExec8(CFsAE* ae, ParamInfo* infoP) {
  ...
  pi.bufH = ae->NewHandle(
      (pi.hor.len + pi.ver.len + pi.dia.len) * sizeof(PF_Pixel) + 1024);
  if (pi.bufH == NULL) {
