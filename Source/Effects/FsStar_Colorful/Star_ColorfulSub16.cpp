...
static PF_Err BlendAdd(refconType refcon, A_long xL, A_long yL, PF_Pixel16* inP,
                       PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  ...
  outP->green =
      RoundShort((A_long)outP->green + (A_long)((PF_FpLong)inP->green * a));
  outP->blue =
      RoundShort((A_long)outP->blue + (A_long)((PF_FpLong)inP->blue * a));
...
static PF_Err CopyAtoTemp(ParamInfo16* infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err TempRev(ParamInfo16* infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err TempSub(ParamInfo16* infoP) {
  PF_Err err = PF_Err_NONE;
  GInfo gi = infoP->gtmp;
...
static PF_Err MaxHor(ParamInfo16* pi) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err MaxVer(ParamInfo16* pi) {
  PF_Err err = PF_Err_NONE;
...
static void MakePixelTable(PF_Pixel16* tbl, A_long len, PF_Pixel16 col,
  ...
  for (A_long i = 0; i < len2; i++) {
    ...
    tbl[i].red = RoundShort(tbl[i].red +
                            (A_long)((PF_FpLong)r * br * (len2 - i) / len2));
    tbl[i].green = RoundShort(tbl[i].green +
                              (A_long)((PF_FpLong)g * br * (len2 - i) / len2));
    tbl[i].blue = RoundShort(tbl[i].blue +
                             (A_long)((PF_FpLong)b * br * (len2 - i) / len2));
  }
}
...
static PF_Err CenterDraw(ParamInfo16* pi) {
  ...
  for (A_long y = 0; y < go.height; y++) {
    for (A_long x = 0; x < go.width; x++) {
      ...
      dataO[posO].alpha = RoundShortFpLong((A_long)dataO[posO].alpha +
                                           (A_long)dataT[posT].green);
...
static void MakeRainbowTable(PF_Pixel16* tbl, A_long len, PF_FpLong ro,
...
PF_Err StarExec16(CFsAE* ae, ParamInfo* infoP) {
  ...
  pi.bufH = ae->NewHandle(
      (pi.hor.len + pi.ver.len + pi.dia.len) * sizeof(PF_Pixel16) + 1024);
  if (pi.bufH == NULL) {
