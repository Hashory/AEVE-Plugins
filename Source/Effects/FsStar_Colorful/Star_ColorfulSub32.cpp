...
static PF_Err BlendAdd(refconType refcon, A_long xL, A_long yL,
                       PF_PixelFloat* inP, PF_PixelFloat* outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err CopyAtoTemp(ParamInfo32* infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err TempRev(ParamInfo32* infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err TempSub(ParamInfo32* infoP) {
  PF_Err err = PF_Err_NONE;
  GInfo gi = infoP->gtmp;
...
static PF_Err MaxHor(ParamInfo32* pi) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err MaxVer(ParamInfo32* pi) {
  PF_Err err = PF_Err_NONE;
...
static void MakePixelTable(PF_PixelFloat* tbl, A_long len, PF_PixelFloat col,
  ...
  for (A_long i = 0; i < len2; i++) {
    ...
    tbl[i].green =
        RoundFpShortDouble(tbl[i].green + (g * br * (len2 - i) / len2));
    tbl[i].blue =
        RoundFpShortDouble(tbl[i].blue + (b * br * (len2 - i) / len2));
  }
}
...
static PF_Err CenterDraw(ParamInfo32* pi) {
  ...
  for (A_long y = 0; y < go.height; y++) {
    for (A_long x = 0; x < go.width; x++) {
      ...
      dataO[posO].alpha =
          RoundFpShortDouble(dataO[posO].alpha + dataT[posT].green);
...
static void MakeRainbowTable(PF_PixelFloat* tbl, A_long len, PF_FpLong ro,
...
PF_Err StarExec32(CFsAE* ae, ParamInfo* infoP) {
  ...
  pi.bufH = ae->NewHandle(
      (pi.hor.len + pi.ver.len + pi.dia.len) * sizeof(PF_PixelFloat) + 1024);
  if (pi.bufH == NULL) {
