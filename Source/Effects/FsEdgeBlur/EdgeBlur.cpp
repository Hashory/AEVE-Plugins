...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // Integer slider bar
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER(STR_BLUR, // Parameter name
  ...
  // Integer slider bar
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER(STR_OFFSET, // Parameter name
                -50,        // Minimum value for numerical input
                50,         // Maximum value for numerical input
                -36,        // Minimum value for slider
                36,         // Maximum value for slider
                0,          // Default value
                ID_OFFSET);
...
static PF_Err HandleChangedParam(
    PF_InData *in_data, PF_OutData *out_data, PF_ParamDef *params[],
    PF_LayerDef *outputP, PF_UserChangedParamExtra *extraP) {
...
static PF_Err QueryDynamicFlags(PF_InData *in_data, PF_OutData *out_data,
  ...
  // to switch whether to draw every frame.
  /*
CFsAE ae;
...
static PF_Err Exec(CFsAE *ae, edgeBlurPrm *infoP) {
  PF_Err err = PF_Err_NONE;
  ...
  if (infoP->blur > 0) {
    ...
    case PF_PixelFormat_ARGB128:
      infoP->scanlineH =
          ae->NewHandle(infoP->scanlineWidth * sizeof(PF_FpShort) * 3 / 2);
      if (infoP->scanlineH) {
        EdgeBlurSub32(infoP);
      } else {
    ...
    case PF_PixelFormat_ARGB64:
      infoP->scanlineH =
          ae->NewHandle(infoP->scanlineWidth * sizeof(A_u_short) * 3 / 2);
      if (infoP->scanlineH) {
        EdgeBlurSub16(infoP);
      } else {
        err = PF_Err_INTERNAL_STRUCT_DAMAGED;
    ...
    case PF_PixelFormat_ARGB32:
      infoP->scanlineH =
          ae->NewHandle(infoP->scanlineWidth * sizeof(A_u_char) * 3 / 2);
      if (infoP->scanlineH) {
        EdgeBlurSub8(infoP);
      } else {
        err = PF_Err_INTERNAL_STRUCT_DAMAGED;
...
DllExport PF_Err EntryPointFunc(PF_Cmd cmd, PF_InData *in_data,
  ...
  try {
    ...
    case PF_Cmd_USER_CHANGED_PARAM:
      err = HandleChangedParam(in_data, out_data, params, output,
