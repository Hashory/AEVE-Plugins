...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_LINE_ONLY, STR_ON, FALSE, 0, ID_LINE_ONLY);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_NEW_COLOR, 0x80, 0x00, 0x00, ID_NEW_COLOR);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_MN_COLOR1_ON, STR_ON, TRUE, 0, ID_MN_COLOR1_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_MN_COLOR1, 0x0F, 0x0F, 0x0F, ID_MN_COLOR1);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_MN_COLOR2_ON, STR_ON, TRUE, 0, ID_MN_COLOR2_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_MN_COLOR2, 0x0A, 0x0A, 0x0A, ID_MN_COLOR2);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_AD_COLOR1_ON, STR_ON, FALSE, 0, ID_AD_COLOR1_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_AD_COLOR1, 0xFF, 0x00, 0x00, ID_AD_COLOR1);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_AD_COLOR2_ON, STR_ON, FALSE, 0, ID_AD_COLOR2_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_AD_COLOR2, 0xFF, 0x00, 0x00, ID_AD_COLOR2);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_AD_COLOR3_ON, STR_ON, FALSE, 0, ID_AD_COLOR3_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_AD_COLOR3, 0xFF, 0x00, 0x00, ID_AD_COLOR3);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_AD_COLOR4_ON, STR_ON, FALSE, 0, ID_AD_COLOR4_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_AD_COLOR4, 0xFF, 0x00, 0x00, ID_AD_COLOR4);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_AD_COLOR5_ON, STR_ON, FALSE, 0, ID_AD_COLOR5_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_AD_COLOR5, 0xFF, 0x00, 0x00, ID_AD_COLOR5);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_AD_COLOR6_ON, STR_ON, FALSE, 0, ID_AD_COLOR6_ON);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_AD_COLOR6, 0xFF, 0x00, 0x00, ID_AD_COLOR6);
...
static PF_Boolean FindMnColor(ParamInfo *infoP, PF_Pixel p) {
  ...
  if (infoP->mnColorMax > 0) {
    for (int i = 0; i < infoP->mnColorMax; i++) {
      if ((p.blue == infoP->mnColor[i].blue) &&
...
static PF_Boolean FindAdColor(ParamInfo *infoP, PF_Pixel p) {
  ...
  if (infoP->adColorMax > 0) {
    for (int i = 0; i < infoP->adColorMax; i++) {
      if ((p.blue == infoP->adColor[i].blue) &&
...
static ParamInfo16 infoTo16(ParamInfo *infoP) {
  ParamInfo16 ret;
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if ((infoP->mnColorMax <= 0) || (infoP->adColorMax <= 0) ||
      (infoP->value <= 0)) {
    return err;
  }
