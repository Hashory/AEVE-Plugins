  ...
  if ((infoP->value > 0)) {
    ...
    for (y = 0; y < cntY; y++) {
      ...
      for (x = 0; x < cntX; x++) {
        ...
        if (F_RAND3(1L << 16) <= infoP->value) {
          pr.a = pr.b = pr.g = pr.r =
              (PF_FpShort)F_RAND2(-prValue, prValue) / PF_MAX_CHAN16;
...
PF_Err GetParams (
                                                PF_InData
*in_data, PF_OutData		*out_data, PF_ParamDef		*params[],
                                                PF_LayerDef		*output,
                                                FsAEParams		*aeprm,
                                                MY_Params
*prm, mosaicPrm			*mprm
...
//入力画像のアドレス if ( (params==NULL)||(output==NULL)||(input==NULL) ) {
                return 0xFF;
        }
        ...
        //モザイクの大きさ
        prm->sizeX	=params[MY_SIZEX]->u.sd.value *
in_data->downsample_x.num / in_data->downsample_x.den; if (prm->sizeX<1)
{prm->sizeX=1;} prm->sizeY	=params[MY_SIZEY]->u.sd.value *
in_data->downsample_x.num / in_data->downsample_x.den; if (prm->sizeY<1)
{prm->sizeY=1;} mprm->mosaicX = prm->sizeX; mprm->mosaicY = prm->sizeY;
...
static PF_Err Render (	PF_InData		*in_data,
                                                PF_OutData
*out_data, PF_ParamDef		*params[], PF_LayerDef		*output )
{
        long	i,j,w,h;
        FsAEParams	aprm;
        MY_Params		prm;
        ...
        if  ( GetParams(in_data,out_data,params,output,&aprm,&prm,&mprm) !=
PF_Err_NONE ) { PF_SPRINTF(	out_data->return_msg,
                                        "[%s]
画像サイズエラーです。御免なさい。", FS_NAME
                                        );
                return err;
        }
