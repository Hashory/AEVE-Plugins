...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, TRUE, 0,
                  ID_TARGET_ENABLED0);
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL0, 0xFF, 0x00, 0x00, ID_TARGET_COL0);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, TRUE, 0,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL1, 0x00, 0xFF, 0x00, ID_TARGET_COL1);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, FALSE, 0,
                  ID_TARGET_ENABLED2);
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL2, 0x00, 0x00, 0xFF, ID_TARGET_COL2);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, FALSE, 0,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL3, 0xFF, 0x00, 0xFF, ID_TARGET_COL3);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, FALSE, 0,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL4, 0x00, 0xFF, 0xFF, ID_TARGET_COL4);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, FALSE, 0,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL5, 0x00, 0x80, 0x80, ID_TARGET_COL5);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, FALSE, 0,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL6, 0x80, 0x00, 0x80, ID_TARGET_COL6);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLE0, STR_TARGET_ENABLE1, FALSE, 0,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL7, 0x80, 0x00, 0x00, ID_TARGET_COL7);
  //----------------------------------------------------------------
...
//-----------------------------------------------------------------------------------
static PF_Err CopyImage8(refconType refcon, A_long xL, A_long yL,
    ...
    if (info->isTargetCopy == TRUE) {
      ...
      if (inP->alpha >= PF_MAX_CHAN8) {
        for (A_long i = 0; i < info->col_count; i++) {
          if ((F_ABS(inP->blue - info->target_col[i].blue) <=
               info->col_range) &&
              (F_ABS(inP->green - info->target_col[i].green) <=
               info->col_range) &&
      ...
      if (b == FALSE) {
        outP->alpha = outP->blue = outP->green = outP->red = 0;
      } else {
...
//-----------------------------------------------------------------------------------
static PF_Err CopyImage16(refconType refcon, A_long xL, A_long yL,
...
//-----------------------------------------------------------------------------------
static PF_Err CopyImage32(refconType refcon, A_long xL, A_long yL,
...
//-----------------------------------------------------------------------------------
static PF_Err GetParams(CFsAE *ae, BlurInfo *infoP) {
  PF_Err err = PF_Err_NONE;
...
//-----------------------------------------------------------------------------------
static void GetBlurPrm(CFsAE *ae, BlurInfo *infoP, blurPrm *bp) {
...
//-----------------------------------------------------------------------------------
static PF_Err GetParams(CFsAE *ae, BlurInfo *infoP, blurPrm *bp) {
  PF_Err err = PF_Err_NONE;
...
//-----------------------------------------------------------------------------------
static PF_Err Exec(CFsAE *ae, BlurInfo *infoP, blurPrm *bp) {
  PF_Err err = PF_Err_NONE;
  ...
  switch (ae->pixelFormat()) {
    case PF_PixelFormat_ARGB128:
      ...
      if (!err) {
        ...
        bp->scanlineH =
            ae->NewHandle(bp->scanlineWidth * sizeof(PF_PixelFloat) * 2);
    ...
    case PF_PixelFormat_ARGB64:
      ...
      if (!err) {
        ...
        bp->scanlineH =
            ae->NewHandle(bp->scanlineWidth * sizeof(PF_Pixel16) * 2);
        if (!bp->scanlineH) return PF_Err_INTERNAL_STRUCT_DAMAGED;
        BlurSub16(bp);
        if (bp->scanlineH != NULL) {
          ae->DisposeHandle(bp->scanlineH);
...
static PF_Err PreRender(PF_InData *in_data, PF_OutData *out_dataP,
