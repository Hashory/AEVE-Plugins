...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_TOPIC(UI_COLOR_TOPIC, ID_COLOR_TOPIC);
  //-----------------
  AEFX_CLR_STRUCT(def);
  ...
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ORG1, UI_ORG2, FALSE, 0, ID_ORG);
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (infoP->value > 0) {
    ...
    ERR(ae->NewWorld(ae->out->width(), ae->out->height() / 4 + 8,
                     PF_PixelFormat_ARGB32, &bw2));

    /*
    CFsBuffer buf = ae->NewBuffer((SPD_RAND_MAX+3)*sizeof(A_u_char));// +
    SPD_RAND_MAX if (buf.alive()==FALSE){ ae->out_data->out_flags |=
    PF_OutFlag_DISPLAY_ERROR_MESSAGE; err = PF_Err_INTERNAL_STRUCT_DAMAGED;
            return err;
    }
    CFsBuffer buf1 = ae->NewBuffer(ae->out->width() * (ae->out->height()+8)
    *sizeof(A_u_char));// + SPD_RAND_MAX if (buf1.alive()==FALSE){
            buf.Dispose();
