...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_UNDER, 0x0, 0x0, 0x0, ID_UNDERCOLOR);
    ...
    if (inP->alpha == 0) {
      *outP = infoP->under;
      outP->alpha = 0;
               ...
               (compPix8Lv(*inP, infoP->color, infoP->range) == TRUE)) {
      *outP = infoP->under;
      outP->alpha = 0;
    } else {
      *outP = *inP;
    ...
    if (inP->alpha == 0) {
      *outP = infoP->under;
      outP->alpha = 0;
               ...
               (compPix16_8Lv(*inP, infoP->color, infoP->range) == TRUE)) {
      *outP = infoP->under;
      outP->alpha = 0;
    } else {
      *outP = *inP;
    ...
    if (inP->alpha == 0) {
      *outP = infoP->under;
      outP->alpha = 0;
               ...
               (compPix32_8Lv(*inP, infoP->color, infoP->range) == TRUE)) {
      *outP = infoP->under;
      outP->alpha = 0;
    } else {
      *outP = *inP;
...
// Main rendering
/*
This function is called for hosts that do not support SmartFX (before After
Effects 7) to render. If you write this function, it will be compatible with
v6.5.
*/
...
//-----------------------------------------------------------------------------------
/*
For SmartFX support, this function is called first to get parameters.
*/
