...
static PF_Err ParamsSetup(PF_InData* in_data, PF_OutData* out_data,
  ...
  PF_ADD_SLIDER(STR_HEIGHT,  // Parameter name
                ...
                16,          // Maximum value of the slider
                2,           // Default value
                ID_HEIGHT);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER(STR_INTER,  // Parameter name
                -100,       // Minimum value for numerical input
                100,        // Maximum value for numerical input
                0,          // Minimum value of the slider
  ...
  if (niP) {
    ...
    if (brt > 0) {
      if (niP->minus == TRUE) {
        ...
        outP->green =
            RoundByteFpLong((PF_FpShort)outP->green - outP->green * brt);
        outP->red = RoundByteFpLong((PF_FpShort)outP->red - outP->red * brt);
      } else {
        outP->blue = RoundByteFpLong((PF_FpShort)outP->blue +
                                     (PF_MAX_CHAN8 - outP->blue) * brt);
        outP->green = RoundByteFpLong((PF_FpShort)outP->green +
                                      (PF_MAX_CHAN8 - outP->green) * brt);
        outP->red = RoundByteFpLong((PF_FpShort)outP->red +
                                    (PF_MAX_CHAN8 - outP->red) * brt);
      }
    }
  }
  ...
  if (niP) {
    ...
    if (brt > 0) {
      if (niP->minus == TRUE) {
        ...
        outP->blue =
            RoundShortFpLong(outP->blue + (PF_MAX_CHAN16 - outP->blue) * brt);
        outP->green =
            RoundShortFpLong(outP->green + (PF_MAX_CHAN16 - outP->green) * brt);
        outP->red =
            RoundShortFpLong(outP->red + (PF_MAX_CHAN16 - outP->red) * brt);
      }
    }
  }
  ...
  if (niP) {
    ...
    if (brt > 0) {
      if (niP->minus == TRUE) {
        ...
        outP->blue = RoundFpShortDouble(outP->blue +
                                        ((PF_FpShort)1.0 - outP->blue) * brt);
        outP->green = RoundFpShortDouble(outP->green +
                                         ((PF_FpShort)1.0 - outP->green) * brt);
        outP->red =
            RoundFpShortDouble(outP->red + ((PF_FpShort)1.0 - outP->red) * brt);
      }
    }
  }
