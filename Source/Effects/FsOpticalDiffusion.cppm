...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_EXTRACT_ENABLED, "", TRUE, 0, ID_EXTRACT_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_START_COLLAPSED;  // これをつけると表示時に開いた状態になる
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_EXTRACT_INVERT, "", TRUE, 0, ID_EXTRACT_INVERT);
  ...
  PF_ADD_FIXED(STR_EXTRACT_COLOR_RANGE,  // パラメータの名前
               ...
               0.5,                      // デフォルトの値
               1,                        // 数値表示に関するフラグ
               0, 0, ID_EXTRACT_COLOR_RANGE);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL1, 0xFF, 0x00, 0x00, ID_EXTRACT_COL1);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL2, 0x00, 0xFF, 0x00, ID_EXTRACT_COL2);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL3, 0x00, 0x00, 0xFF, ID_EXTRACT_COL3);

  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL4, 0xFF, 0xFF, 0x00, ID_EXTRACT_COL4);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL5, 0xFF, 0x00, 0xFF, ID_EXTRACT_COL5);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL6, 0x00, 0xFF, 0xFF, ID_EXTRACT_COL6);

  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL7, 0xFF, 0x80, 0x00, ID_EXTRACT_COL7);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_EXTRACT_COL8, 0x00, 0x80, 0xFF, ID_EXTRACT_COL8);
...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (infoP->blendOpacity < 0)
    infoP->blendOpacity = 0;
  else if (infoP->blendOpacity > 1)
    infoP->blendOpacity = 1;
  ...
  if (infoP->blendOpacity <= 0) {
    ...
    infoP->noiseValue =
        (PF_FpShort)(infoP->noiseValue * infoP->blendOpacity / 0.25);
  }
