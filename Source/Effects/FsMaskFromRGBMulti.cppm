...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_CHECKBOX(STR_R, STR_ON, TRUE, 0, ID_R);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_SUPERVISE | PF_ParamFlag_CANNOT_TIME_VARY |
              PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(STR_G, STR_ON, FALSE, 0, ID_G);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_SUPERVISE | PF_ParamFlag_CANNOT_TIME_VARY |
              PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(STR_B, STR_ON, FALSE, 0, ID_B);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_SUPERVISE | PF_ParamFlag_CANNOT_TIME_VARY |
              PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(STR_Y, STR_ON, FALSE, 0, ID_Y);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_SUPERVISE | PF_ParamFlag_CANNOT_TIME_VARY |
              PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(STR_M, STR_ON, FALSE, 0, ID_M);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_SUPERVISE | PF_ParamFlag_CANNOT_TIME_VARY |
              PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(STR_C, STR_ON, FALSE, 0, ID_C);
  //----------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
static PF_Err Mask8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                    PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ...
  if (col.alpha == 0) {
    ...
    if ((niP->colFlag[M_COL::Y]) || (niP->colFlag[M_COL::M]) ||
        (niP->colFlag[M_COL::C])) {
    ...
    if (niP->colFlag[M_COL::R]) {
      // ax = (A_long)col.blue  + (A_long)col.green - (A_long)col.blue  *
      // (A_long)col.green /  PF_MAX_CHAN8;
      ax = MAX((A_long)col.blue, (A_long)col.green);
      bx = (A_long)col.red;
      v += RoundByteFpLong((double)(bx - ax) * niP->col_level);
    }
    if ((niP->colFlag[M_COL::G]) && (v < PF_MAX_CHAN8)) {
      // ax = (A_long)col.blue  + (A_long)col.red - (A_long)col.blue  *
      // (A_long)col.red /  PF_MAX_CHAN8;
      ax = MAX((A_long)col.blue, (A_long)col.red);
      bx = (A_long)col.green;
      v += RoundByteFpLong((double)(bx - ax) * niP->col_level);
    }
    if ((niP->colFlag[M_COL::B]) && (v < PF_MAX_CHAN8)) {
      // ax = (A_long)col.green  + (A_long)col.red - (A_long)col.green  *
      // (A_long)col.red /  PF_MAX_CHAN8;
    ...
    if ((niP->colFlag[M_COL::Y]) && (v < PF_MAX_CHAN8)) {
      // ax = (A_long)revc.green  + (A_long)revc.red - (A_long)revc.green  *
      // (A_long)revc.red /  PF_MAX_CHAN8;
      ax = MAX((A_long)revc.green, (A_long)revc.red);
      bx = (A_long)revc.blue;
      v += RoundByteFpLong((double)(bx - ax) * niP->col_level);
    }
    if ((niP->colFlag[M_COL::M]) && (v < PF_MAX_CHAN8)) {
      // ax = (A_long)revc.blue  + (A_long)revc.red - (A_long)revc.blue  *
      // (A_long)revc.red /  PF_MAX_CHAN8;
    ...
    if ((niP->colFlag[M_COL::C]) && (v < PF_MAX_CHAN8)) {
      // ax = (A_long)revc.blue  + (A_long)revc.green - (A_long)revc.blue  *
      // (A_long)revc.green /  PF_MAX_CHAN8;
...
//-------------------------------------------------------------------------------------------------
static PF_Err Mask16(refconType refcon, A_long xL, A_long yL, PF_Pixel16 *inP,
                     PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
...
//-------------------------------------------------------------------------------------------------
static PF_Err Mask32(refconType refcon, A_long xL, A_long yL,
                     PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
