  ...
  try {
    ...
    if ((!err) && (in_data->appl_id != 'PrMr')) {
      ...
      if (!err) {
        ...
        for (A_long i = PixelSelector_NUM; i < PixelSelector_POP; i++)
          hide_themB[i] = TRUE;
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_TOPIC(UI_TOPIC, PixelSelector_TOPIC);
  //-----------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_REV, UI_ON, FALSE, 0, PixelSelector_REV);
  //-----------------
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_SUPERVISE | PF_ParamFlag_CANNOT_TIME_VARY |
              PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(UI_FILL, UI_ON, FALSE, 0, PixelSelector_FILL);
  //-----------------
  AEFX_CLR_STRUCT(def);
  ...
  AEFX_CLR_STRUCT(def);
  def.ui_flags = PF_PUI_DISABLED;
  PF_ADD_PERCENT(UI_FILL_OPA, 100, PixelSelector_FILL_OPACITY);
  //-----------------
  AEFX_CLR_STRUCT(def);
  ...
  for (i = 0; i < PS_PRM_COUNT; i++) {
    ...
    AEFX_CLR_STRUCT(def);
    def.flags = PF_ParamFlag_START_COLLAPSED;  //
    PF_ADD_TOPIC(num, PARAMS_IDX(i, paramsOffset_topic));
    //-----------------
    AEFX_CLR_STRUCT(def);
    ...
    //-----------------
    AEFX_CLR_STRUCT(def);
    PF_END_TOPIC(PARAMS_IDX(i, paramsOffset_topic_end));
    //*************************************************
  }
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  for (A_long i = 0; i < PS_PRM_COUNT; i++) {
    ...
    if ((!err) && (flg == TRUE)) {
      ERR(ae->GetCOLOR(PARAMS_IDX(i, paramsOffset_s_color),
                       &infoP->src[count]));
      if (!err)
        count++;
      else
