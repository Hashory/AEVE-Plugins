...
inline long lineLen32(PF_Fixed a, PF_Fixed b) {
  double tx, ty, tl;
  tx = FIX_2_FLOAT(a);
  ty = FIX_2_FLOAT(b);
  tl = sqrt(tx * tx + ty * ty);
      ...
      if (oD[targetOut].blue == 1) {
        if ((A_long)F_RAND() < t0) {
          for (k = 0; k <= prm->point_count; k++) {
            td.start.x = xx + ln0 +
                         (long)(F_RAND() * (ln1 + 1.0f) / (F_RAND_MAX + 1.0f));
            td.start.y = yy + ln0 +
                         (long)(F_RAND() * (ln1 + 1.0f) / (F_RAND_MAX + 1.0f));

            td.length_i =
                ir0 + (long)(F_RAND() * (ir1 + 1.0f) / (1.0f + F_RAND_MAX));
            td.length_o =
                or0 + (long)(F_RAND() * (or1 + 1.0f) / (1.0f + F_RAND_MAX));
            td.opacity =
                p0 + (long)(F_RAND() * (p1 + 1.0f) / (1.0f + F_RAND_MAX));
            ;
