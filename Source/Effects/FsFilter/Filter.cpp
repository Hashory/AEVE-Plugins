...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_BASE_ENABLED, STR_ON, TRUE, 0, ID_BASE_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
...
#pragma region enabled
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_CHECKBOX(num, STR_ON, FLT_Enabled[i], 0, ID_NUM(i, ID_BASE_ENABLED));
#pragma endregion
    //----------------------------------------------------------------
...
#pragma region extract
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_POPUP(num,
                 STR_EXTRACT_COUNT,    // Number of menus
                 FLT_EXTRACT_MODE[i],  // Default
                 STR_EXTRACT_ITEMS, ID_NUM(i, ID_EXTRACT));
#pragma endregion
    //----------------------------------------------------------------
...
#pragma region max
    ...
    PF_ADD_SLIDER(num,         // Parameter name
                  0,           // Minimum value for numerical input
                  50,          // Maximum value for numerical input
                  0,           // Minimum value of slider
                  20,          // Maximum value of slider
                  FLT_MAX[i],  // Default value
                  ID_NUM(i, ID_MAX));
#pragma endregion
    //----------------------------------------------------------------
...
#pragma region blend
    ...
    PF_ADD_POPUP(num,
                 ...
                 FLT_BLEND_MODE[i],  // Default
                 STR_BLEND_ITEMS, ID_NUM(i, ID_BLEND));
#pragma endregion
    //----------------------------------------------------------------
...
#pragma endregion
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_ALPHA_ON, STR_ON, FALSE, 0, ID_ALPHA_ON);
  //----------------------------------------------------------------
...
static PF_Err GetParams(CAE *ae, ParamInfo *infoP) {
  ...
  for (int i = 0; i < PARAMSET_COUNT; i++) {
    ERR(ae->GetCHECKBOX(ID_NUM(i, ID_BASE_ENABLED),
                        &infoP->paramset[i].enabled));
    ...
    ERR(ae->GetFLOAT(ID_NUM(i, ID_SOFTNESS_HI),
    ...
    ERR(ae->GetFLOAT(ID_NUM(i, ID_SOFTNESS_LO),
                     &infoP->paramset[i].softness_lo));
    ...
    if (infoP->paramset[i].opacity < 0)
      infoP->paramset[i].opacity = 0;
    else if (infoP->paramset[i].opacity > 1)
      infoP->paramset[i].opacity = 1;
...
//-------------------------------------------------------------------------------------------------
static PF_Err Exec(CAE *ae, ParamInfo *infoP) {
  PF_Err err = PF_Err_NONE;
