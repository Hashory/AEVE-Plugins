...
static PF_Err BaseCopy8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                        PF_Pixel8 *outP) {
...
PF_Err Exec08(CAE *ae, ParamInfo *infoP) {
  ...
  if ((infoP->paramset[0].enabled == FALSE) &&
    ...
    // Create buffer
    ERR(ae->NewWorld(ae->output->width, ae->output->height,
                     PF_PixelFormat_ARGB64, worldP));
    ...
    for (A_long i = 0; i < PARAMSET_COUNT; i++) {
      ...
      // Transfer to out Blend processing
