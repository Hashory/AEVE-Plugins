...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if (niP->alpha == 0) {
      ...
      outP->alpha = RoundByteFpShort((PF_FpShort)inP->alpha +
                                     (PF_FpShort)inP->alpha * niP->alpha);
    } else {
      outP->alpha = RoundByteFpShort((PF_FpShort)inP->alpha +
                                     (PF_FpShort)(PF_MAX_CHAN8 - inP->alpha) *
                                         niP->alpha);
    }
    ...
    if (niP->green == 0) {
      ...
      outP->green = RoundByteFpShort((PF_FpShort)inP->green +
                                     (PF_FpShort)inP->green * niP->green);
    } else {
      outP->green = RoundByteFpShort((PF_FpShort)inP->green +
                                     (PF_FpShort)(PF_MAX_CHAN8 - inP->green) *
                                         niP->green);
    }
    if (niP->blue == 0) {
      outP->blue = inP->blue;
    } else if (niP->blue < 0) {
      outP->blue = RoundByteFpShort((PF_FpShort)inP->blue +
                                    (PF_FpShort)inP->blue * niP->blue);
    } else {
      outP->blue =
          RoundByteFpShort((PF_FpShort)inP->blue +
                           (PF_FpShort)(PF_MAX_CHAN8 - inP->blue) * niP->blue);
    }
    if (niP->red == 0) {
      outP->red = inP->red;
    } else if (niP->red < 0) {
      outP->red = RoundByteFpShort((PF_FpShort)inP->red +
                                   (PF_FpShort)inP->red * niP->red);
    } else {
      outP->red =
          RoundByteFpShort((PF_FpShort)inP->red +
                           (PF_FpShort)(PF_MAX_CHAN8 - inP->red) * niP->red);
    }
  }
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if (niP->alpha == 0) {
      ...
      outP->alpha = RoundShortFpLong((PF_FpShort)inP->alpha +
                                     (PF_FpShort)inP->alpha * niP->alpha);
    } else {
      outP->alpha = RoundShortFpLong((PF_FpShort)inP->alpha +
                                     (PF_FpShort)(PF_MAX_CHAN16 - inP->alpha) *
                                         niP->alpha);
    }
    ...
    if (niP->green == 0) {
      ...
      outP->green = RoundShortFpLong((PF_FpShort)inP->green +
                                     (PF_FpShort)inP->green * niP->green);
    } else {
      outP->green = RoundShortFpLong((PF_FpShort)inP->green +
                                     (PF_FpShort)(PF_MAX_CHAN16 - inP->green) *
                                         niP->green);
    }
    if (niP->blue == 0) {
      outP->blue = inP->blue;
    } else if (niP->blue < 0) {
      outP->blue = RoundShortFpLong((PF_FpShort)inP->blue +
                                    (PF_FpShort)inP->blue * niP->blue);
    } else {
      outP->blue =
          RoundShortFpLong((PF_FpShort)inP->blue +
                           (PF_FpShort)(PF_MAX_CHAN16 - inP->blue) * niP->blue);
    }
    if (niP->red == 0) {
      outP->red = inP->red;
    } else if (niP->red < 0) {
      outP->red = RoundShortFpLong((PF_FpShort)inP->red +
                                   (PF_FpShort)inP->red * niP->red);
    } else {
      outP->red =
          RoundShortFpLong((PF_FpShort)inP->red +
                           (PF_FpShort)(PF_MAX_CHAN16 - inP->red) * niP->red);
    }
  }
...
static PF_Err FilterImage32(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if (niP->alpha == 0) {
      ...
      outP->alpha = RoundFpShort((PF_FpShort)inP->alpha +
                                 (PF_FpShort)inP->alpha * niP->alpha);
    } else {
      outP->alpha =
          RoundFpShort((PF_FpShort)inP->alpha +
                       (PF_FpShort)((PF_FpShort)1 - inP->alpha) * niP->alpha);
    }

    if (niP->green == 0) {
      outP->green = inP->green;
    } else if (niP->green < 0) {
      outP->green = RoundFpShort((PF_FpShort)inP->green +
                                 (PF_FpShort)inP->green * niP->green);
    } else {
      outP->green =
          RoundFpShort((PF_FpShort)inP->green +
                       (PF_FpShort)((PF_FpShort)1 - inP->green) * niP->green);
    }
    if (niP->blue == 0) {
      outP->blue = inP->blue;
    } else if (niP->blue < 0) {
      outP->blue = RoundFpShort((PF_FpShort)inP->blue +
                                (PF_FpShort)inP->blue * niP->blue);
    } else {
      outP->blue =
          RoundFpShort((PF_FpShort)inP->blue +
                       (PF_FpShort)((PF_FpShort)1 - inP->blue) * niP->blue);
    }
    if (niP->red == 0) {
      outP->red = inP->red;
    } else if (niP->red < 0) {
      outP->red =
          RoundFpShort((PF_FpShort)inP->red + (PF_FpShort)inP->red * niP->red);
    } else {
      outP->red =
          RoundFpShort((PF_FpShort)inP->red +
                       (PF_FpShort)((PF_FpShort)1 - inP->red) * niP->red);
    }
  }
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if ((infoP->alpha == 0) && (infoP->red == 0) && (infoP->green == 0) &&
      (infoP->blue == 0)) {
    ERR(ae->CopyInToOut());
  } else {
