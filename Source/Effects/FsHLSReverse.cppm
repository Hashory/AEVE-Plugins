...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_HUE, STR_REV, FALSE, 0, ID_HUE);
  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_LUM, STR_REV, TRUE, 0, ID_LUM);

  //----------------------------------------------------------------
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_SAT, STR_REV, FALSE, 0, ID_SAT);
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    ...
    if (niP->org_blend == 0) {
      ...
      outP->red = RoundByteFpLong((PF_FpLong)inP->red * niP->org_blend +
                                  (PF_FpLong)r * k);
      outP->green = RoundByteFpLong((PF_FpLong)inP->green * niP->org_blend +
                                    (PF_FpLong)g * k);
      outP->blue = RoundByteFpLong((PF_FpLong)inP->blue * niP->org_blend +
                                   (PF_FpLong)b * k);
    }
  }
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    ...
    if (niP->org_blend == 0) {
      ...
      PF_FpLong k = 1 - niP->org_blend;
      outP->red = RoundShortFpLong((PF_FpLong)inP->red * niP->org_blend +
                                   (PF_FpLong)r * k);
      outP->green = RoundShortFpLong((PF_FpLong)inP->green * niP->org_blend +
                                     (PF_FpLong)g * k);
      outP->blue = RoundShortFpLong((PF_FpLong)inP->blue * niP->org_blend +
                                    (PF_FpLong)b * k);
    }
  }
...
static PF_Err FilterImage32(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    ...
    if (niP->org_blend == 0) {
      ...
      PF_FpLong k = 1 - niP->org_blend;
      outP->red = RoundFpShortDouble((PF_FpLong)inP->red * niP->org_blend +
                                     (PF_FpLong)r * k);
      outP->green = RoundFpShortDouble((PF_FpLong)inP->green * niP->org_blend +
                                       (PF_FpLong)g * k);
      outP->blue = RoundFpShortDouble((PF_FpLong)inP->blue * niP->org_blend +
                                      (PF_FpLong)b * k);
    }
  }
