...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_ALPHA_REV, STR_ON, FALSE, 0, ID_REV);

  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLED0, STR_ON, FALSE, 0, ID_TARGET_ENABLE0);

  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COLOR0, 0xFF, 0x00, 0x00, ID_TARGET_COLOR0);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLED1, STR_ON, FALSE, 0, ID_TARGET_ENABLE1);

  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COLOR1, 0x00, 0xFF, 0x00, ID_TARGET_COLOR1);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLED2, STR_ON, FALSE, 0, ID_TARGET_ENABLE2);

  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COLOR2, 0x00, 0x00, 0xFF, ID_TARGET_COLOR2);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET_ENABLED3, STR_ON, FALSE, 0, ID_TARGET_ENABLE3);

  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COLOR3, 0xFF, 0x00, 0xFF, ID_TARGET_COLOR3);
...
static PF_Err ColorMatKey8(refconType refcon, A_long xL, A_long yL,
  ...
  for (A_long idx = 0; idx < 4; idx++) {
    if (infoP->ci[idx].target_enabled == TRUE) {
      v = 1.0 - (PF_SQRT(PF_POW(lab.L - infoP->ci[idx].target_lab.L, 2) +
      ...
      if (infoP->ci[idx].target_softness == 0) {
        ...
        if (v <= infoP->ci[idx].target_start) {
          ...
          v = (v - infoP->ci[idx].target_start) /
              (infoP->ci[idx].target_softness);
        }
      }
      v2 += v;
      if (v2 > 1.0) v2 = 1.0;
...
static PF_Err ColorMatKey16(refconType refcon, A_long xL, A_long yL,
...
static PF_Err ColorMatKey32(refconType refcon, A_long xL, A_long yL,
...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  for (A_long idx = 0; idx < 4; idx++) {
    ...
    infoP->ci[idx].target_start =
        infoP->ci[idx].target_border - infoP->ci[idx].target_softness;
...
// Main rendering
/*
    Hosts that do not support SmartFX (older than After Effects 7) call this
   function to draw. Writing this function makes it compatible with v6.5.
*/
