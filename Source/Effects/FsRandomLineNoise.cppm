...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_COLOR1, STR_COLOR2, FALSE, 0, ID_COLOR);
  //----------------------------------------------------------------
  // ４個目のパラメータ
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  ...
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_VERTUAL1, STR_VERTUAL2, FALSE, 0, ID_VERTUAL);
...
static void drawLine32(LineDrawParam *lP) {
  ...
  for (j = lP->y0; j <= lP->y1; j++) {
    for (i = lP->x0; i <= lP->x1; i++) {
      if (data[p].alpha != 0) {
        data[p].red =
            RoundFpShortDouble(data[p].red + (PF_FpShort)lP->r / PF_MAX_CHAN16);
        data[p].green = RoundFpShortDouble(data[p].green +
                                           (PF_FpShort)lP->g / PF_MAX_CHAN16);
        data[p].blue = RoundFpShortDouble(data[p].blue +
                                          (PF_FpShort)lP->b / PF_MAX_CHAN16);
      }
      p++;
    }
    p += o;
...
//-------------------------------------------------------------------------------------------------
static PF_Boolean GetLineDrawParamsFromParamInfo(ParamInfo *infoP,
                                                 LineDrawParam *lP) {
  PF_Boolean ret = TRUE;
  ...
  if (ret == TRUE) {
    ...
    if ((lP->x1 < 0) || (lP->x0 >= lP->width) || (lP->y1 < 0) ||
        (lP->y0 >= lP->height))
      ret = FALSE;
    if (ret == TRUE) {
      if (lP->x0 < 0) lP->x0 = 0;
...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (!err) {
    ...
    if (infoP->lengthMin > infoP->lengthMax)
      swapLong(&infoP->lengthMin, &infoP->lengthMax);
  }
