...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_Main_Color, 0x00, 0x00, 0x00, ID_Main_Color);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  ...
  for (A_long j = 0; j < h; j++) {
    ...
    for (A_long i = 1; i < w - 1; i++) {
      if (compPix8Lv(infoP->Main_Color, scanline[i], infoP->lv) == TRUE) {
        ...
        if (compPix8Lv(infoP->Main_Color, scanline[i - 1], infoP->lv) ==
            FALSE) {
          d = scanline[i - 1];
          dv = pxV8(d);
        }
    ...
    if (compPix8Lv(infoP->Main_Color, data[adrY + w - 1], infoP->lv) == TRUE) {
      if (compPix8Lv(infoP->Main_Color, data[adrY + w - 2], infoP->lv) ==
          FALSE) {
  ...
  for (A_long i = 0; i < w; i++) {
    ...
    for (A_long j = 1; j < h - 1; j++) {
      if (compPix8Lv(infoP->Main_Color, scanline[j], infoP->lv) == TRUE) {
        ...
        if (compPix8Lv(infoP->Main_Color, scanline[j - 1], infoP->lv) ==
            FALSE) {
          d = scanline[j - 1];
          dv = pxV8(d);
        }
    ...
    if (compPix8Lv(infoP->Main_Color, data[i + ay], infoP->lv) == TRUE) {
      if (compPix8Lv(infoP->Main_Color, data[i + ay - wt], infoP->lv) ==
          FALSE) {
  ...
  for (A_long j = 0; j < h; j++) {
    ...
    for (A_long i = 1; i < w - 1; i++) {
      if (compPix16_8Lv(scanline[i], infoP->Main_Color, infoP->lv) == TRUE) {
        ...
        if (compPix16_8Lv(scanline[i - 1], infoP->Main_Color, infoP->lv) ==
            FALSE) {
          d = scanline[i - 1];
          dv = pxV16(d);
        }
    ...
    if (compPix16_8Lv(data[adrY], infoP->Main_Color, infoP->lv) == TRUE) {
      if (compPix16_8Lv(data[adrY + 1], infoP->Main_Color, infoP->lv) ==
          FALSE) {
        data[adrY] = data[adrY + 1];
      } else {
        cnt++;
    ...
    if (compPix16_8Lv(data[adrY + w - 1], infoP->Main_Color, infoP->lv) ==
        TRUE) {
      if (compPix16_8Lv(data[adrY + w - 2], infoP->Main_Color, infoP->lv) ==
          FALSE) {
        data[adrY + w - 1] = data[adrY + w - 2];
      } else {
        cnt++;
  ...
  for (A_long i = 0; i < w; i++) {
    ...
    for (A_long j = 1; j < h - 1; j++) {
      if (compPix16_8Lv(scanline[j], infoP->Main_Color, infoP->lv) == TRUE) {
        ...
        if (compPix16_8Lv(scanline[j - 1], infoP->Main_Color, infoP->lv) ==
            FALSE) {
          d = scanline[j - 1];
          dv = pxV16(d);
        }
    ...
    if (compPix16_8Lv(data[i + ay], infoP->Main_Color, infoP->lv) == TRUE) {
      if (compPix16_8Lv(data[i + ay - wt], infoP->Main_Color, infoP->lv) ==
          FALSE) {
        data[i + ay] = data[i + ay - wt];
      } else {
        cnt++;
  ...
  for (A_long j = 0; j < h; j++) {
    ...
    for (A_long i = 1; i < w - 1; i++) {
      if (compPix32_8Lv(scanline[i], infoP->Main_Color, infoP->lv) == TRUE) {
        ...
        if (compPix32_8Lv(scanline[i - 1], infoP->Main_Color, infoP->lv) ==
            FALSE) {
          d = scanline[i - 1];
          dv = pxV32(d);
        }
    ...
    if (compPix32_8Lv(data[adrY], infoP->Main_Color, infoP->lv) == TRUE) {
      if (compPix32_8Lv(data[adrY + 1], infoP->Main_Color, infoP->lv) ==
          FALSE) {
        data[adrY] = data[adrY + 1];
      } else {
        cnt++;
    ...
    if (compPix32_8Lv(data[adrY + w - 1], infoP->Main_Color, infoP->lv) ==
        TRUE) {
      if (compPix32_8Lv(data[adrY + w - 2], infoP->Main_Color, infoP->lv) ==
          FALSE) {
        data[adrY + w - 1] = data[adrY + w - 2];
      } else {
        cnt++;
  ...
  for (A_long i = 0; i < w; i++) {
    ...
    for (A_long j = 1; j < h - 1; j++) {
      if (compPix32_8Lv(scanline[j], infoP->Main_Color, infoP->lv) == TRUE) {
        ...
        if (compPix32_8Lv(scanline[j - 1], infoP->Main_Color, infoP->lv) ==
            FALSE) {
          d = scanline[j - 1];
          dv = pxV32(d);
        }
    ...
    if (compPix32_8Lv(data[i + ay], infoP->Main_Color, infoP->lv) == TRUE) {
      if (compPix32_8Lv(data[i + ay - wt], infoP->Main_Color, infoP->lv) ==
          FALSE) {
        data[i + ay] = data[i + ay - wt];
      } else {
        cnt++;
