...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_CHECKBOX(STR_ISHALF, "on", FALSE, 0, ID_ISHALF);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  outP->red =
      RoundByteLong((A_long)((double)niP->weightR * a / PF_MAX_CHAN8 + 0.5));
  outP->green =
      RoundByteLong((A_long)((double)niP->weightG * a / PF_MAX_CHAN8 + 0.5));
