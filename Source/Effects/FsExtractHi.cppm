...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_POPUP(STR_TARGET,
               ...
               STR_TARGET_ITEM, ID_TARGET);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  // def.flags =
  // PF_ParamFlag_CANNOT_TIME_VARY;//これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_CUSTUM_COLOR, 0xFF, 0x00, 0x00, ID_CUSTUM_COLOR);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  // def.flags = PF_ParamFlag_SUPERVISE |
  // PF_ParamFlag_CANNOT_TIME_VARY |
  // PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(STR_INVERT, STR_INVERT_ON, FALSE, 0, ID_INVERT);
  //----------------------------------------------------------------
...
static PF_FpLong levelLuminunce8(PF_Pixel8 *col, ParamInfo *infoP) {
  return ((0.29891 * (PF_FpLong)col->red) + (0.58661 * (PF_FpLong)col->green) +
          (0.11448 * (PF_FpLong)col->blue)) /
         PF_MAX_CHAN8;
}
//-------------------------------------------------------------------------------------------------
...
static PF_FpLong levelLuminunce32(PF_Pixel32 *col, ParamInfo *infoP) {
  PF_FpLong r = (0.29891 * (PF_FpLong)col->red) +
                (0.58661 * (PF_FpLong)col->green) +
                (0.11448 * (PF_FpLong)col->blue);
...
static PF_FpLong levelRed8(PF_Pixel8 *col, ParamInfo *infoP) {
  return ((PF_FpLong)col->red -
          ((PF_FpLong)col->green + (PF_FpLong)col->blue) / 2) /
         PF_MAX_CHAN8;
}
//-------------------------------------------------------------------------------------------------
...
static PF_FpLong levelGreen8(PF_Pixel8 *col, ParamInfo *infoP) {
  return ((PF_FpLong)col->green -
          ((PF_FpLong)col->red + (PF_FpLong)col->blue) / 2) /
         PF_MAX_CHAN8;
}
//-------------------------------------------------------------------------------------------------
...
static PF_FpLong levelBlue8(PF_Pixel8 *col, ParamInfo *infoP) {
  return ((PF_FpLong)col->blue -
          ((PF_FpLong)col->red + (PF_FpLong)col->green) / 2) /
         PF_MAX_CHAN8;
}
//-------------------------------------------------------------------------------------------------
...
static PF_FpLong levelRGBMax8(PF_Pixel8 *col, ParamInfo *infoP) {
  PF_FpLong v =
      (PF_FpLong)(MAX(MAX(col->red, col->green), col->blue)) / PF_MAX_CHAN8;
...
static PF_FpLong levelRGBMax16(PF_Pixel16 *col, ParamInfo *infoP) {
  PF_FpLong v =
      (PF_FpLong)(MAX(MAX(col->red, col->green), col->blue)) / PF_MAX_CHAN16;
...
static PF_FpLong levelColor8(PF_Pixel8 *col, ParamInfo *infoP) {
  ...
  PF_FpLong v = sqrt(pow((r - infoP->r), 2) + pow((g - infoP->g), 2) +
                     pow((b - infoP->b), 2));
  v /= sqrt(3);
  if (v > 1) v = 1;
  v = 1.0 - v;
