...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRC, 0xFF, 0xFF, 0xFF, ID_SRC_COLOR);

  // Specify color
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DST, 0x00, 0x00, 0x00, ID_DST_COLOR);
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if ((ABS(outP->red - niP->src.red) <= niP->level) &&
        (ABS(outP->green - niP->src.green) <= niP->level) &&
        (ABS(outP->blue - niP->src.blue) <= niP->level)) {
...
static PF_Err FilterImage8Zero(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if ((outP->red == niP->src.red) && (outP->green == niP->src.green) &&
        (outP->blue == niP->src.blue)) {
      outP->red = niP->dst.red;
      outP->green = niP->dst.green;
      outP->blue = niP->dst.blue;
    }
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    ...
    if ((ABS(p.red - niP->src.red) <= niP->level) &&
        (ABS(p.green - niP->src.green) <= niP->level) &&
        (ABS(p.blue - niP->src.blue) <= niP->level)) {
...
static PF_Err FilterImage16Zero(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    PF_Pixel p = CONV16TO8(*outP);
    if ((p.red == niP->src.red) && (p.green == niP->src.green) &&
        (p.blue == niP->src.blue)) {
      outP->red = niP->dst16.red;
      outP->green = niP->dst16.green;
      outP->blue = niP->dst16.blue;
