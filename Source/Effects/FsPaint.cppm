...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // チェックボックス
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(StrEXECUTED_CB1, StrEXECUTED_CB2, TRUE, 0, ID_EXECUTED_CB);
  ...
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(StrCOLOR, 0xFF, 0x00, 0x00, ID_COLOR);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
...
static PF_Err GetParams(CFsAE *ae, PaintInfo *infoP) {
  ...
  if (!err) {
    ...
    if (!err)
      infoP->color.alpha = RoundByteLong(PF_MAX_CHAN8 * infoP->opacity >> 16);
  }
  return err;
}
//-------------------------------------------------------------------------------------------------
static PF_Err Exec(CFsAE *ae, PaintInfo *infoP) {
  PF_Err err = PF_Err_NONE;
