...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_CHECKBOX(STR_NOISE_FRAME1, STR_NOISE_FRAME2, TRUE, 0, ID_NOISE_FRAME);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  ...
  def.flags = PF_ParamFlag_SUPERVISE | PF_ParamFlag_CANNOT_TIME_VARY |
              PF_ParamFlag_CANNOT_INTERP;
  PF_ADD_CHECKBOX(STR_HIDDEN_ON1, STR_HIDDEN_ON2, FALSE, 0, ID_HIDDEN_ON);
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_CHECKBOX1, STR_CHECKBOX2, FALSE, 0, ID_CHECKBOX);
  //----------------------------------------------------------------
  // 角度
  AEFX_CLR_STRUCT(def);
  ...
  try {
    ...
    if ((!err) && (in_data->appl_id != 'PrMr')) {
      ...
      if (!err) {
        ...
        if (b) {
          for (A_long i = ID_HIDDEN_ON + 1; i < ID_NUM_PARAMS; i++)
            hide_themB[i] = TRUE;
        }
        for (A_long i = 1; i < ID_NUM_PARAMS; i++)
          ERR(ae.SetDynamicStreamFlag(i, AEGP_DynStreamFlag_HIDDEN,
                                      hide_themB[i]));
      }
      //--------------------
      if (!err) {
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if ((infoP->r == 0) && (infoP->g == 0) && (infoP->b == 0) &&
      (infoP->noise <= 0)) {
    return err;
  }
