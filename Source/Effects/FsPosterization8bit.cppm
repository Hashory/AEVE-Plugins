...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_G, "on", TRUE, 0, ID_GRAY);
  //----------------------------------------------------------------
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  // まずアルファーの処理
  A_long va = (A_long)((double)infoP->level * a +
                       0.5);  // levelが最大になるように四捨五入して整数整数化
  va = (A_long)((double)va * PF_MAX_CHAN8 / (double)infoP->level +
                0.5);  // 255に変換
  outP->alpha = RoundByteLong(va);

  if (infoP->grayOnly == TRUE) {
    double y = (0.299 * r + 0.587 * g + 0.114 * b);
    A_long vy = (A_long)((double)infoP->level * y +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vy = (A_long)((double)vy * PF_MAX_CHAN8 / (double)infoP->level +
                  0.5);                                   // 255に変換
    vy = (A_long)((double)vy * PF_MAX_CHAN8 / va + 0.5);  // Matの処理
    outP->blue = outP->green = outP->red = RoundByteLong(vy);
  } else {
    A_long vr = (A_long)((double)infoP->level * r +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vr = (A_long)((double)vr * PF_MAX_CHAN8 / (double)infoP->level +
                  0.5);                                   // 255に変換
    vr = (A_long)((double)vr * PF_MAX_CHAN8 / va + 0.5);  // Matの処理

    A_long vg = (A_long)((double)infoP->level * g +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vg = (A_long)((double)vg * PF_MAX_CHAN8 / (double)infoP->level +
                  0.5);                                   // 255に変換
    vg = (A_long)((double)vg * PF_MAX_CHAN8 / va + 0.5);  // Matの処理

    A_long vb = (A_long)((double)infoP->level * b +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vb = (A_long)((double)vb * PF_MAX_CHAN8 / (double)infoP->level +
                  0.5);                                   // 255に変換
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  A_long va = (A_long)((double)infoP->level * a +
                       0.5);  // levelが最大になるように四捨五入して整数整数化
  va = (A_long)((double)va * PF_MAX_CHAN16 / (double)infoP->level +
                0.5);  // 255に変換
  ...
  if (infoP->grayOnly == TRUE) {
    ...
    A_long vy = (A_long)((double)infoP->level * y +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vy = (A_long)((double)vy * PF_MAX_CHAN16 / (double)infoP->level +
                  0.5);                                    // 255に変換
    ...
    A_long vr = (A_long)((double)infoP->level * r +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vr = (A_long)((double)vr * PF_MAX_CHAN16 / (double)infoP->level +
                  0.5);                                    // 255に変換
    vr = (A_long)((double)vr * PF_MAX_CHAN16 / va + 0.5);  // Matの処理

    A_long vg = (A_long)((double)infoP->level * g +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vg = (A_long)((double)vg * PF_MAX_CHAN16 / (double)infoP->level +
                  0.5);                                    // 255に変換
    vg = (A_long)((double)vg * PF_MAX_CHAN16 / va + 0.5);  // Matの処理

    A_long vb = (A_long)((double)infoP->level * b +
                         0.5);  // levelが最大になるように四捨五入して整数整数化
    vb = (A_long)((double)vb * PF_MAX_CHAN16 / (double)infoP->level +
                  0.5);                                    // 255に変換
