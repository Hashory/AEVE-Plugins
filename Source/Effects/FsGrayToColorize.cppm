...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_COLOR(STR_COL1, 0xFF, 0xF9, 0x9C, ID_COL1);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_COL2,  // F5DD6D
               0xF5, 0xDD, 0x6D, ID_COL2);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_COL3,  // F3A447
               0xF3, 0xA4, 0x47, ID_COL3);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_COL4,  // DB9039
               0xDB, 0x90, 0x38, ID_COL4);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_COL5,  // F3C186
               0xF3, 0xC1, 0x86, ID_COL5);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_CHECKBOX(STR_MAT_CB, STR_ON, FALSE, 0, ID_MAT_CB);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_COLOR(STR_MAT_COL, 0xFF, 0xFF, 0xFF, ID_MAT_COLOR);
  //----------------------------------------------------------------
...
static A_long sampleCompare(PF_Pixel s, PF_Pixel d) {
  double b = sqrt(pow((double)s.red, 2) + pow((double)s.green, 2) +
                  pow((double)s.blue, 2));
  return (A_long)((100.0 * b) + 0.5);
}
//-------------------------------------------------------------------------------------------------
static PF_Err exec8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                    PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ...
  if (niP->mat_on) {
    ...
    c.red = RoundByteLong((A_long)c.alpha * ((A_long)c.red - (A_long)m.red) /
                              PF_MAX_CHAN8 +
                          (A_long)m.red);
    c.green = RoundByteLong(
        (A_long)c.alpha * ((A_long)c.green - (A_long)m.green) / PF_MAX_CHAN8 +
        (A_long)m.green);
    c.blue = RoundByteLong((A_long)c.alpha * ((A_long)c.blue - (A_long)m.blue) /
                               PF_MAX_CHAN8 +
                           (A_long)m.blue);
  }
  //******************
  ...
  //******************
  A_long v =
      (A_long)((0.29891 * (PF_FpLong)c.red) + (0.58661 * (PF_FpLong)c.green) +
               (0.11448 * (PF_FpLong)c.blue) + 0.5);
  v = FROM8_SAMPLE(v);
  if (v < 0)
    v = 0;
  else if (v > SAMPLE_MAX)
    v = SAMPLE_MAX;
...
//-------------------------------------------------------------------------------------------------
static PF_Err exec16(refconType refcon, A_long xL, A_long yL, PF_Pixel16 *inP,
                     PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ...
  if (niP->mat_on) {
    ...
    c.red = RoundByteLong((A_long)c.alpha * ((A_long)c.red - (A_long)m.red) /
                          ...
                          (A_long)m.red);
    c.green = RoundByteLong(
        (A_long)c.alpha * ((A_long)c.green - (A_long)m.green) / PF_MAX_CHAN16 +
        (A_long)m.green);
    c.blue = RoundByteLong((A_long)c.alpha * ((A_long)c.blue - (A_long)m.blue) /
                               PF_MAX_CHAN16 +
                           (A_long)m.blue);
...
//-------------------------------------------------------------------------------------------------
static PF_Err exec32(refconType refcon, A_long xL, A_long yL,
                     PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
