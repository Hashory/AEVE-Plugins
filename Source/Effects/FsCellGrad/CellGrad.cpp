...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL1, 0xFF, 0xFF, 0xFF, ID_TARGET_COL1);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL2, 0xFF, 0x00, 0x00, ID_TARGET_COL2);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL3, 0x00, 0xFF, 0x00, ID_TARGET_COL3);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL4, 0x00, 0x00, 0xFF, ID_TARGET_COL4);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL5, 0xFF, 0xFF, 0x00, ID_TARGET_COL5);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL6, 0xFF, 0x00, 0xFF, ID_TARGET_COL6);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL7, 0x80, 0x80, 0x00, ID_TARGET_COL7);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET_COL8, 0x80, 0x00, 0x80, ID_TARGET_COL8);
  AEFX_CLR_STRUCT(def);
  ...
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_START_COL, 0xFF, 0xFF, 0xFF, ID_START_COL);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_LAST_COL, 0x00, 0x00, 0x00, ID_LAST_COL);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_GUIDE_SHOW, STR_ON, FALSE, 0, ID_GUIDE_SHOW);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_GUIDE_COLOR, 0xFF, 0x00, 0x00, ID_GUIDE_COL);
...
static LineInfo RotC(PF_InData *in_data, LineInfo li) { return li; }
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  FRectGrad rg(ae->in_data, rct, infoP->angle, infoP->startOver,
               infoP->lastOver);
  ...
  if (infoP->guideShow == TRUE) {
    ae->out->Line(rct.left, rct.top, rct.right, rct.top, infoP->guideCol);
    ae->out->Line(rct.right, rct.top, rct.right, rct.bottom, infoP->guideCol);
    ae->out->Line(rct.right, rct.bottom, rct.left, rct.bottom, infoP->guideCol);
    ae->out->Line(rct.left, rct.bottom, rct.left, rct.top, infoP->guideCol);

    ae->out->Line((A_long)rg.start().x, (A_long)rg.start().y,
                  (A_long)rg.last().x, (A_long)rg.last().y, infoP->guideCol);
  }
...
/*
    This function is called for hosts that do not support SmartFX (like After
   Effects 7 or earlier) to render the effect. Writing this function ensures
...
/*
    For SmartFX-compatible hosts, this function is called first to acquire
   parameters.
*/
