...
static PF_Err (*ToGrayFunc8)(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel8 *inP, PF_Pixel8 *outP);

static PF_Err (*ToGrayFunc16)(refconType refcon, A_long xL, A_long yL,
                              PF_Pixel16 *inP, PF_Pixel16 *outP);

static PF_Err (*ToGrayFunc32)(refconType refcon, A_long xL, A_long yL,
...
// Notify After Effects of the parameters.
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_POPUP(STR_GRAY_MODE_POP1,
               STR_GRAY_MODE_COUNT,  // Number of menu items
               STR_GRAY_MODE_DEF,    // Default item
               STR_GRAY_MODE_POP2, ID_GRAY_MODE);

  // Fixed-point slider
  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(STR_ORG_BLEND,  // Parameter name
...
inline PF_Pixel GMIX8(PF_Pixel *s, A_long v, PF_FpShort p) {
  ...
  if (p <= 0) {
    r.alpha = s->alpha;
    r.red = r.green = r.blue = (A_u_char)v;
  } else {
    PF_FpShort d0 = p;
    PF_FpShort d1 = 1 - p;
    r.alpha = s->alpha;
...
static PF_Err ToGrayFunc8_yuv(refconType refcon, A_long xL, A_long yL,
                              PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
    A_long v = (A_long)((0.29891 * (PF_FpLong)inP->red) +
                        (0.58661 * (PF_FpLong)inP->green) +
                        (0.11448 * (PF_FpLong)inP->blue));
    *outP = GMIX8(inP, v, niP->org_blend);
  }
  return err;
}

static PF_Err ToGrayFunc8_hls(refconType refcon, A_long xL, A_long yL,
                              PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
    A_long v = (MAX(MAX(inP->red, inP->green), inP->blue) +
                MIN(MIN(inP->red, inP->green), inP->blue)) >>
               1;
    *outP = GMIX8(inP, v, niP->org_blend);
  }
...
static PF_Err ToGrayFunc8_rgb(refconType refcon, A_long xL, A_long yL,
                              PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc8_r(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc8_g(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc8_b(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc8_rgbmax(refconType refcon, A_long xL, A_long yL,
                                 PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc8_lab(refconType refcon, A_long xL, A_long yL,
                              PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
inline PF_Pixel16 GMIX16(PF_Pixel16 *s, A_long v, PF_FpShort p) {
  ...
  if (p <= 0) {
    r.alpha = s->alpha;
    r.red = r.green = r.blue = (A_u_short)v;
  } else {
    PF_FpShort d0 = p;
    PF_FpShort d1 = 1 - p;
    r.alpha = s->alpha;
    ...
    r.green =
        RoundShortFpLong(((PF_FpShort)s->green * d0 + (PF_FpShort)v * d1));
    r.blue = RoundShortFpLong(((PF_FpShort)s->blue * d0 + (PF_FpShort)v * d1));
  }
  return r;
}

static PF_Err ToGrayFunc16_yuv(refconType refcon, A_long xL, A_long yL,
                               PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc16_hls(refconType refcon, A_long xL, A_long yL,
                               PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc16_rgb(refconType refcon, A_long xL, A_long yL,
                               PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc16_r(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc16_g(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc16_b(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc16_rgbmax(refconType refcon, A_long xL, A_long yL,
                                  PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc16_lab(refconType refcon, A_long xL, A_long yL,
                               PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
inline PF_Pixel32 GMIX32(PF_PixelFloat *s, PF_FpLong v, PF_FpShort p) {
  ...
  if (p <= 0) {
    r.alpha = s->alpha;
    r.red = r.green = r.blue = (PF_FpShort)v;
  } else {
    PF_FpShort d0 = p;
    PF_FpShort d1 = 1 - p;
    r.alpha = s->alpha;
    ...
    r.green =
        RoundFpShortDouble(((PF_FpShort)s->green * d0 + (PF_FpShort)v * d1));
    r.blue =
        RoundFpShortDouble(((PF_FpShort)s->blue * d0 + (PF_FpShort)v * d1));
  }
  return r;
}

static PF_Err ToGrayFunc32_yuv(refconType refcon, A_long xL, A_long yL,
                               PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
    PF_FpShort v = (PF_FpShort)((0.29891 * (PF_FpLong)inP->red) +
                                (0.58661 * (PF_FpLong)inP->green) +
                                (0.11448 * (PF_FpLong)inP->blue));
...
static PF_Err ToGrayFunc32_hls(refconType refcon, A_long xL, A_long yL,
                               PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
    PF_FpShort v = (MAX(MAX(inP->red, inP->green), inP->blue) +
                    MIN(MIN(inP->red, inP->green), inP->blue)) /
                   2;
    *outP = GMIX32(inP, v, niP->org_blend);
  }
...
static PF_Err ToGrayFunc32_rgb(refconType refcon, A_long xL, A_long yL,
                               PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc32_r(refconType refcon, A_long xL, A_long yL,
                             PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc32_g(refconType refcon, A_long xL, A_long yL,
                             PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc32_b(refconType refcon, A_long xL, A_long yL,
                             PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc32_rgbmax(refconType refcon, A_long xL, A_long yL,
                                  PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err ToGrayFunc32_lab(refconType refcon, A_long xL, A_long yL,
                               PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  if (niP) {
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (infoP->org_blend < 1) {
    switch (infoP->gray_mode) {  // #define STR_GRAY_MODE_POP2
                                 //"(Y)uv|h(L)s|(R+G+B)/3|(R)gb|r(G)b|rg(B)"
      case 1:
