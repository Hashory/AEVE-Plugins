...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // これをつけるとキーフレームが撃てなくなる
  PF_ADD_CHECKBOX(STR_DISPMAP, "on", FALSE, 0, ID_DISPMAP);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
static PF_Err dispMap8(refconType refcon, A_long xL, A_long yL, PF_Pixel *inP,
                       PF_Pixel *outP) {
  PF_Err err = PF_Err_NONE;
  outP->red = outP->green = outP->blue = outP->alpha;
  outP->alpha = inP->alpha;
  return err;
}
static PF_Err dispMap16(refconType refcon, A_long xL, A_long yL,
...
//-------------------------------------------------------------------------------------------------
static PF_Err dispMap32(refconType refcon, A_long xL, A_long yL,
...
//-------------------------------------------------------------------------------------------------
static PF_Err inToAlphap8(refconType refcon, A_long xL, A_long yL,
...
//-------------------------------------------------------------------------------------------------
static PF_Err inToAlphap16(refconType refcon, A_long xL, A_long yL,
                           PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  outP->alpha = inP->alpha;
  return err;
...
//-------------------------------------------------------------------------------------------------
static PF_Err inToAlphap32(refconType refcon, A_long xL, A_long yL,
                           PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  outP->alpha = inP->alpha;
  return err;
...
*/
//-------------------------------------------------------------------------------------------------
static PF_Err clearHalf8(refconType refcon, A_long xL, A_long yL, PF_Pixel *inP,
                         PF_Pixel *outP) {
  PF_Err err = PF_Err_NONE;
  ...
  *outP = back;
  if (niP->noise > 0) {
    if (xorShift() > 0x3fff) {
      A_long a = (A_long)(((double)PF_MAX_CHAN8 -
                           2 * (double)PF_MAX_CHAN8 * xorShiftDouble()) *
                          niP->noise);
...
//-------------------------------------------------------------------------------------------------
static PF_Err clearHalf16(refconType refcon, A_long xL, A_long yL,
                          PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  PF_Pixel16 back = {PF_HALF_CHAN16, PF_HALF_CHAN16, PF_HALF_CHAN16,
                     PF_HALF_CHAN16};
  *outP = back;
  if (niP->noise > 0) {
    if (xorShift() > 0x3fff) {
      A_long a = (A_long)(((double)PF_MAX_CHAN16 -
                           2 * (double)PF_MAX_CHAN16 * xorShiftDouble()) *
                          niP->noise);
...
//-------------------------------------------------------------------------------------------------
static PF_Err clearHalf32(refconType refcon, A_long xL, A_long yL,
                          PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  PF_PixelFloat back = {PF_HALF_CHAN32, PF_HALF_CHAN32, PF_HALF_CHAN32,
                        PF_HALF_CHAN32};
  ParamInfo *niP = reinterpret_cast<ParamInfo *>(refcon);
  *outP = back;
  if (niP->noise > 0) {
...
*/
//-------------------------------------------------------------------------------------------------
static PF_Err clearRed8(refconType refcon, A_long xL, A_long yL, PF_Pixel *inP,
                        PF_Pixel *outP) {
  PF_Err err = PF_Err_NONE;
...
//-------------------------------------------------------------------------------------------------
static PF_Err clearRed16(refconType refcon, A_long xL, A_long yL,
                         PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
//-------------------------------------------------------------------------------------------------
static PF_Err clearRed32(refconType refcon, A_long xL, A_long yL,
                         PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
...
static void Draw8(ParamInfo *infoP, DrawInfo *di, int idx) {
  int rndTable[6][8] = {
      {0, 0, 0, 1, 0, 1, 0, 1}, {2, 3, 2, 1, 2, 1, 2, 1},
      {4, 5, 4, 3, 4, 5, 2, 3}, {6, 7, 6, 7, 6, 5, 4, 5},
      {8, 8, 8, 9, 5, 7, 7, 6}, {10, 10, 10, 10, 10, 9, 8, 7}};
 ...
 */
//-------------------------------------------------------------------------------------------------
static PF_Err alphatoToRed8(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel *inP, PF_Pixel *outP) {
  PF_Err err = PF_Err_NONE;
...
//-------------------------------------------------------------------------------------------------
static PF_Err alphatoToRed16(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
//-------------------------------------------------------------------------------------------------
static PF_Err alphatoToRed32(refconType refcon, A_long xL, A_long yL,
                             PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  infoP->map[0].size -= 1;
  if (infoP->map[0].size < 0)
    infoP->map[0].size = 0;
  else if (infoP->map[0].size > 5)
    infoP->map[0].size = 5;
  ...
  infoP->map[1].size -= 1;
  if (infoP->map[1].size < 0)
    infoP->map[1].size = 0;
  else if (infoP->map[1].size > 5)
    infoP->map[1].size = 5;
  ...
  infoP->map[2].size -= 1;
  if (infoP->map[2].size < 0)
    infoP->map[2].size = 0;
  else if (infoP->map[2].size > 5)
    infoP->map[2].size = 5;
...
static PF_Err Exec8(CFsAE *ae, ParamInfo *infoP) {
  ...
  if ((infoP->map[idx].strong > 0) && (infoP->map[idx].value > 0)) {
    Draw8(infoP, &di, idx);
    rt.SetRotLength(infoP->rot,
                    (PF_Fixed)((double)len * infoP->map[idx].speed));
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if ((infoP->map[0].value <= 0) && (infoP->map[1].value <= 0) &&
      (infoP->map[2].value <= 0) && (infoP->noise <= 0) && (infoP->level <= 0))
    b = true;
  if ((infoP->map[0].strong <= 0) && (infoP->map[1].strong <= 0) &&
      (infoP->map[2].strong <= 0) && (infoP->noise <= 0) && (infoP->level <= 0))
    b = true;

  if (b) {
    if (infoP->disp_map) {
      PF_Pixel back = {PF_MAX_CHAN8, PF_HALF_CHAN8, PF_HALF_CHAN8,
                       PF_HALF_CHAN8};
      ae->out->clear(back);
