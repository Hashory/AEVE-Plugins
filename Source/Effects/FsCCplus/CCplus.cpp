...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(StrORG_REV1, StrORG_REV2, TRUE, 0, ID_ORG_REV);

  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(StrCOLOR_START, 0xFF, 0xFF, 0xFF, ID_COLOR_START);

  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(StrCOLOR_CENTER, 0x80, 0x80, 0xFF, ID_COLOR_CENTER);

  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(StrCOLOR_END, 0x0, 0x0, 0xFF, ID_COLOR_END);
...
PF_Pixel colorValue8(CCPInfo *infoP, A_u_char value) {
  ...
  if (value == infoP->color_border) {
    ...
    A_long v1 = max - value;
    A_long v2 = value;
    r.blue = RoundByteLong(infoP->color_start.blue * v1 / max +
                           infoP->color_center.blue * v2 / max);
    r.green = RoundByteLong(infoP->color_start.green * v1 / max +
                            infoP->color_center.green * v2 / max);
    r.red = RoundByteLong(infoP->color_start.red * v1 / max +
                          infoP->color_center.red * v2 / max);
  } else {
    A_long max = PF_MAX_CHAN8 - infoP->color_border;
    A_long v1 = max - (value - infoP->color_border);
    A_long v2 = value - infoP->color_border;
    r.blue = RoundByteLong(infoP->color_center.blue * v1 / max +
                           infoP->color_end.blue * v2 / max);
    r.green = RoundByteLong(infoP->color_center.green * v1 / max +
                            infoP->color_end.green * v2 / max);
    r.red = RoundByteLong(infoP->color_center.red * v1 / max +
                          infoP->color_end.red * v2 / max);
  }
  r.alpha = RoundByteLong(infoP->alpha_start +
                          (infoP->alpha_end - infoP->alpha_start) * value /
                              PF_MAX_CHAN8);
  return r;
}
...
PF_Pixel16 colorValue16(CCPInfo16 *infoP, A_u_short value) {
  ...
  if (value == infoP->color_border) {
    ...
    A_long v1 = max - value;
    A_long v2 = value;
    r.blue = RoundShort(infoP->color_start.blue * v1 / max +
                        infoP->color_center.blue * v2 / max);
    r.green = RoundShort(infoP->color_start.green * v1 / max +
                         infoP->color_center.green * v2 / max);
    r.red = RoundShort(infoP->color_start.red * v1 / max +
                       infoP->color_center.red * v2 / max);
  } else {
    A_long max = PF_MAX_CHAN16 - infoP->color_border;
    A_long v1 = max - (value - infoP->color_border);
    A_long v2 = value - infoP->color_border;
    r.blue = RoundShortFpLong(infoP->color_center.blue * v1 / max +
                              infoP->color_end.blue * v2 / max);
    r.green = RoundShortFpLong(infoP->color_center.green * v1 / max +
                               infoP->color_end.green * v2 / max);
    r.red = RoundShortFpLong(infoP->color_center.red * v1 / max +
                             infoP->color_end.red * v2 / max);
  }
  r.alpha = RoundShortFpLong(infoP->alpha_start +
                             (infoP->alpha_end - infoP->alpha_start) * value /
                                 PF_MAX_CHAN16);
  return r;
}
...
PF_PixelFloat colorValue32(CCPInfo32 *infoP, PF_FpShort value) {
  ...
  if (value == infoP->color_border) {
    ...
    r.blue = RoundFpShortDouble(
        (infoP->color_start.blue * v1 + infoP->color_center.blue * v2) / max);
    r.green = RoundFpShortDouble(
        (infoP->color_start.green * v1 + infoP->color_center.green * v2) / max);
    r.red = RoundFpShortDouble(
        (infoP->color_start.red * v1 + infoP->color_center.red * v2) / max);
  } else {
    ...
    r.blue = RoundFpShortDouble(
        (infoP->color_center.blue * v1 + infoP->color_end.blue * v2) / max);
    r.green = RoundFpShortDouble(
        (infoP->color_center.green * v1 + infoP->color_end.green * v2) / max);
    r.red = RoundFpShortDouble(
        (infoP->color_center.red * v1 + infoP->color_end.red * v2) / max);
  }
  r.alpha = RoundFpShortDouble(infoP->alpha_start +
                               (infoP->alpha_end - infoP->alpha_start) * value);
  return r;
}
...
static PF_Err AddColor8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                        PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err AddColor16(refconType refcon, A_long xL, A_long yL,
...
static PF_Err AddColor32(refconType refcon, A_long xL, A_long yL,
...
static PF_Err AddNoise8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                        PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err AddNoise16(refconType refcon, A_long xL, A_long yL,
                         PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
#if defined(SUPPORT_SMARTFX)
static PF_Err AddNoise32(refconType refcon, A_long xL, A_long yL,
                         PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ...
  if (info) {
    PF_FpShort v;
    if (outP->alpha > 0) {
      v = (PF_FpShort)F_RAND2(-info->noise_value, info->noise_value) /
          PF_MAX_CHAN16;
...
static PF_Err Exec(CFsAE *ae, CCPInfo *infoP) {
  PF_Err err = PF_Err_NONE;
