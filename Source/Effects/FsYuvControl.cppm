...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_FLOAT_SLIDER(STR_Y,  // Name
                      ...
                      ID_Y);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_UV_AUTO, STR_ON, FALSE, 0, ID_UV_AUTO);
  AEFX_CLR_STRUCT(def);
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (rr < 0)
    rr = 0;
  else if (rr > PF_MAX_CHAN8)
    rr = PF_MAX_CHAN8;
  if (gg < 0)
    gg = 0;
  else if (gg > PF_MAX_CHAN8)
    gg = PF_MAX_CHAN8;
  if (bb < 0)
    bb = 0;
  else if (bb > PF_MAX_CHAN8)
    bb = PF_MAX_CHAN8;
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  if (rr < 0)
    rr = 0;
  else if (rr > PF_MAX_CHAN16)
    rr = PF_MAX_CHAN16;
  if (gg < 0)
    gg = 0;
  else if (gg > PF_MAX_CHAN16)
    gg = PF_MAX_CHAN16;
  if (bb < 0)
    bb = 0;
  else if (bb > PF_MAX_CHAN16)
    bb = PF_MAX_CHAN16;
