...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  for (A_long dc = 0; dc < infoP->drawCount; dc++) {
    ...
    for (A_long fc = 0; fc < infoP->foldCount; fc++) {
      ld.Points.CalcCenterPos(infoP->lineSize, infoP->lineMove, infoP->offset,
                              depth);
      depth++;
    }
    ...
    while (ff > 2) {
      ...
      for (A_long fc = 0; fc < ff; fc++) {
        ld.Points.CalcCenterPos2(infoP->lineSize * 0.7, infoP->lineMove * 4 / 3,
                                 infoP->offset, depth, pos - 1);
        rcm += 3;
        depth++;
      }
      ld.Points.Wipe(infoP->wipe);
