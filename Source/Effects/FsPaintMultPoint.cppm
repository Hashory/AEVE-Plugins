...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  for (A_long i = 0; i < POINT_COLOR_COUNT; i++) {
    ...
    PF_ADD_CHECKBOX(cap, STR_EXECUTED_CB2, FALSE, 0, ID_UI_CB(i));
    AEFX_CLR_STRUCT(def);
    ...
    PF_ADD_CHECKBOX(cap, STR_EXECUTED_CB2, FALSE, 0, ID_UI_GID(i));

    AEFX_CLR_STRUCT(def);
    PF_END_TOPIC(ID_UI_TOPIC_END(i));
  }
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_GUIDE_ALL_CB1, STR_GUIDE_ALL_CB2, FALSE, 0,
                  ID_UI_GUIDE_VISIBLE);
  //----------------------------------------------------------------
