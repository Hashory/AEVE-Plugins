...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //-----------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ANCHOR_ENABLED1, UI_ANCHOR_ENABLED2, FALSE, 0,
                  ID_ANCHOR_ENABLED);
  //-----------------
  AEFX_CLR_STRUCT(def);
  ...
  //-----------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_POINT_LEN_SYSNC1, UI_POINT_LEN_SYSNC2, TRUE, 0,
                  ID_POINT_LEN_SYNC);
  //-----------------
  // ポップアップ
  AEFX_CLR_STRUCT(def);
...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (!err) infoP->radius = ae->downScale(infoP->radius);
  ...
  if (!err) infoP->point_length = ae->downScale(infoP->point_length);
