...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // Specify color
  AEFX_CLR_STRUCT(def);
  def.flags = PF_ParamFlag_CANNOT_TIME_VARY;  // Cannot set keyframes with this
  PF_ADD_COLOR(STR_BASE_COLOR, 0x00, 0x00, 0x00, ID_BASE_COLOR);
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if (inP->alpha <= 0) {
      ...
      outP->red = (niP->base_color.red * (PF_MAX_CHAN8 - inP->alpha) +
                   inP->red * inP->alpha) /
                  PF_MAX_CHAN8;
      outP->green = (niP->base_color.green * (PF_MAX_CHAN8 - inP->alpha) +
                     inP->green * inP->alpha) /
                    PF_MAX_CHAN8;
      outP->blue = (niP->base_color.blue * (PF_MAX_CHAN8 - inP->alpha) +
                    inP->blue * inP->alpha) /
                   PF_MAX_CHAN8;
    }
  }
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if (inP->alpha <= 0) {
      ...
      outP->red = (niP->base_color.red * (PF_MAX_CHAN16 - inP->alpha) +
                   inP->red * inP->alpha) /
                  PF_MAX_CHAN16;
      outP->green = (niP->base_color.green * (PF_MAX_CHAN16 - inP->alpha) +
                     inP->green * inP->alpha) /
                    PF_MAX_CHAN16;
      outP->blue = (niP->base_color.blue * (PF_MAX_CHAN16 - inP->alpha) +
                    inP->blue * inP->alpha) /
                   PF_MAX_CHAN16;
    }
  }
...
static PF_Err FilterImage32(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    if (inP->alpha <= 0) {
      ...
      outP->alpha = 1.0;
      outP->red = (PF_FpShort)(niP->base_color.red * (1.0 - inP->alpha) +
                               inP->red * inP->alpha);
      outP->green = (PF_FpShort)(niP->base_color.green * (1.0 - inP->alpha) +
                                 inP->green * inP->alpha);
      outP->blue = (PF_FpShort)(niP->base_color.blue * (1.0 - inP->alpha) +
                                inP->blue * inP->alpha);
    }
  }
  return err;
...
static PF_Err Render(PF_InData *in_dataP, PF_OutData *out_data,
                     PF_ParamDef *params[], PF_LayerDef *output) {
  PF_Err err = PF_Err_NONE;
