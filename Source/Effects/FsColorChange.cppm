...
PF_Err (*subFunc8)(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                   PF_Pixel8 *outP);
PF_Err (*subFunc16)(refconType refcon, A_long xL, A_long yL, PF_Pixel16 *inP,
                    PF_Pixel16 *outP);
PF_Err (*subFunc32)(refconType refcon, A_long xL, A_long yL, PF_PixelFloat *inP,
                    PF_PixelFloat *outP);
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL0, 255, 255, 255, ID_SRCCOL0);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL0, 128, 128, 128, ID_DSTCOL0);

  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET1, STR_EXEC, FALSE, 0, ID_TARGET1);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL1, 255, 0, 0, ID_SRCCOL1);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL1, 128, 0, 0, ID_DSTCOL1);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET2, STR_EXEC, FALSE, 0, ID_TARGET2);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL2, 0, 255, 0, ID_SRCCOL2);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL2, 0, 128, 0, ID_DSTCOL2);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET3, STR_EXEC, FALSE, 0, ID_TARGET3);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL3, 0, 0, 255, ID_SRCCOL3);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL3, 0, 0, 128, ID_DSTCOL3);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET4, STR_EXEC, FALSE, 0, ID_TARGET4);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL4, 255, 255, 0, ID_SRCCOL4);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL4, 128, 128, 0, ID_DSTCOL4);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET5, STR_EXEC, FALSE, 0, ID_TARGET5);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL5, 0, 255, 255, ID_SRCCOL5);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL5, 0, 128, 128, ID_DSTCOL5);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET6, STR_EXEC, FALSE, 0, ID_TARGET6);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL6, 255, 0, 255, ID_SRCCOL6);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL6, 128, 0, 128, ID_DSTCOL0);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_TARGET7, STR_EXEC, FALSE, 0, ID_TARGET7);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SRCCOL7, 0, 0, 0, ID_SRCCOL7);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DSTCOL7, 255, 255, 255, ID_DSTCOL7);
...
//-----------------------------------------------------------------------------------
static PF_Err FilterImage8Zero(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    for (int i = 0; i < niP->count; i++) {
      if ((inP->red == niP->Src[i].red) && (inP->green == niP->Src[i].green) &&
          (inP->blue == niP->Src[i].blue)) {
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    for (int i = 0; i < niP->count; i++) {
      if ((ABS(inP->red - niP->Src[i].red) <= niP->Lv) &&
          (ABS(inP->green - niP->Src[i].green) <= niP->Lv) &&
          (ABS(inP->blue - niP->Src[i].blue) <= niP->Lv)) {
        outP->alpha = inP->alpha;
        outP->red = niP->Dst[i].red;
        outP->green = niP->Dst[i].green;
        outP->blue = niP->Dst[i].blue;
...
//-----------------------------------------------------------------------------------
static PF_Err FilterImage16Zero(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    ...
    for (int i = 0; i < niP->count; i++) {
      if ((p.red == niP->Src[i].red) && (p.green == niP->Src[i].green) &&
          (p.blue == niP->Src[i].blue)) {
        outP->alpha = inP->alpha;
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  if (niP) {
    PF_Pixel p = CONV16TO8(*inP);
    for (int i = 0; i < niP->count; i++) {
      if ((ABS(p.red - niP->Src[i].red) <= niP->Lv) &&
          (ABS(p.green - niP->Src[i].green) <= niP->Lv) &&
          (ABS(p.blue - niP->Src[i].blue) <= niP->Lv)) {
        outP->alpha = inP->alpha;
        outP->red = niP->Dst16[i].red;
        outP->green = niP->Dst16[i].green;
        outP->blue = niP->Dst16[i].blue;
...
//-----------------------------------------------------------------------------------
static PF_Err FilterImage32Zero(refconType refcon, A_long xL, A_long yL,
