...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  col = infoP->ae->in->getPixelFloat8(pdx, pdy);
  g = col.green;
  a = RoundByteLong((A_long)a + (A_long)col.alpha -
                    (A_long)a * (A_long)col.alpha / PF_MAX_CHAN8);
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  col = infoP->ae->in->getPixelFloat16(pdx, pdy);
  g = col.green;
  a = RoundShort((A_long)a + (A_long)col.alpha -
                 (A_long)a * (A_long)col.alpha / PF_MAX_CHAN16);
  ...
  /*
          ...
          PF_MAX_CHAN16-(PF_MAX_CHAN16*PF_MAX_CHAN16 - PF_MAX_CHAN16*b) + (a
     *PF_MAX_CHAN16 - a *b)

          PF_MAX_CHAN16-PF_MAX_CHAN16*PF_MAX_CHAN16 + PF_MAX_CHAN16*b + a
     *PF_MAX_CHAN16 - a *b PF_MAX_CHAN16(1-PF_MAX_CHAN16 + b + a - a
     *b/PF_MAX_CHAN16)
...
static PF_Err FilterImage32(refconType refcon, A_long xL, A_long yL,
  ...
  col = infoP->ae->in->getPixelFloat32(pdx, pdy);
  g = col.green;
  a = RoundFpShortDouble((double)a + (double)col.alpha -
