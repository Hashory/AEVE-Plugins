...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ParamDef def;
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_ON, STR_ON2, TRUE, 0, ID_ON);

  // Specify position
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos0", 00, 00, 0, ID_POS0);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos1", 10, 10, 0, ID_POS1);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos2", 20, 20, 0, ID_POS2);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos3", 30, 30, 0, ID_POS3);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos4", 40, 40, 0, ID_POS4);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos5", 50, 50, 0, ID_POS5);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos6", 60, 60, 0, ID_POS6);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos7", 70, 70, 0, ID_POS7);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos8", 80, 80, 0, ID_POS8);
  AEFX_CLR_STRUCT(def);
  PF_ADD_POINT("pos9", 90, 90, 0, ID_POS9);
...
// rendering main
/*
 * For hosts that do not support SmartFX (before After Effects 7), this function
 * is called for rendering. If you write this function, it will be compatible
 * with v6.5.
 */
...
/*
 * For SmartFX, this function is called first to get the parameters.
 */
