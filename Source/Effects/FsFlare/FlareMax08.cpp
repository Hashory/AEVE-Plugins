...
static PF_Err MaxRedHor(CFsGraph* g, A_long max, PF_Handle bufH) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    A_u_char mv = 0;
    // Store the maximum value in scanline mv
    for (A_long i = 0; i < w; i++) {
      scanline[i] = data[ypos + i].red;
      if (mv < scanline[i]) mv = scanline[i];
    }
    if (mv <= 0) continue;  // Not necessary if the maximum value is 0
...
static PF_Err MaxAllHor(CFsGraph* g, A_long max, PF_Handle bufH) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    for (A_long i = 0; i < w; i++) {
      ...
      scanlineLv[i] = RoundByteLong(((A_long)data[ypos + i].red +
                                     (A_long)data[ypos + i].green +
                                     (A_long)data[ypos + i].blue) /
                                    3);
...
static PF_Err MaxAllVur(CFsGraph* g, A_long max, PF_Handle bufH) {
  ...
  for (A_long x = 0; x < w; x++) {
    ...
    for (A_long i = 0; i < h; i++) {
      A_long p = x + ypos;
      scanline[i] = data[p];
      scanlineLv[i] = RoundByteLong(
          ((A_long)data[p].red + (A_long)data[p].green + (A_long)data[p].blue) /
          3);
      if (mv < scanlineLv[i]) mv = scanlineLv[i];
      ypos += wt;
    }
