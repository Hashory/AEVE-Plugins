...
static PF_Err BaseCopyImage(refconType refcon, A_long xL, A_long yL,
...
static PF_Err BaseCopyWhiteBase(refconType refcon, A_long xL, A_long yL,
                                PF_Pixel8* inP, PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
  ...
  if (a >= 1) {
    ...
    outP->red =
        RoundByteDouble(v * (double)infoP->BaseColor.red / PF_MAX_CHAN8);
    outP->green =
        RoundByteDouble(v * (double)infoP->BaseColor.green / PF_MAX_CHAN8);
    outP->blue =
        RoundByteDouble(v * (double)infoP->BaseColor.blue / PF_MAX_CHAN8);
...
static PF_Err BaseCopyBlackBase(refconType refcon, A_long xL, A_long yL,
                                PF_Pixel8* inP, PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
static PF_Err BaseCopyAlphaBase(refconType refcon, A_long xL, A_long yL,
                                PF_Pixel8* inP, PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
static PF_Err WhiteBase(refconType refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                        PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err BlackBase(refconType refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                        PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  FlareInfo* infoP = reinterpret_cast<FlareInfo*>(refcon);
...
static PF_Err AlphaBase(refconType refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                        PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  FlareInfo* infoP = reinterpret_cast<FlareInfo*>(refcon);
...
static PF_Err ImageBase(refconType refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                        PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  FlareInfo* infoP = reinterpret_cast<FlareInfo*>(refcon);
...
static PF_Err BlendScreen(refconType refcon, A_long xL, A_long yL,
  ...
  FlareInfo* infoP = reinterpret_cast<FlareInfo*>(refcon);
  outP->red =
      RoundByteLong((A_long)inP->red + (A_long)outP->red -
                    ((A_long)inP->red * (A_long)outP->red) / PF_MAX_CHAN8);
  outP->green =
      RoundByteLong((A_long)inP->green + (A_long)outP->green -
                    ((A_long)inP->green * (A_long)outP->green) / PF_MAX_CHAN8);
  outP->blue =
      RoundByteLong((A_long)inP->blue + (A_long)outP->blue -
                    ((A_long)inP->blue * (A_long)outP->blue) / PF_MAX_CHAN8);
  outP->alpha =
      RoundByteLong((A_long)inP->alpha + (A_long)outP->alpha -
                    ((A_long)inP->alpha * (A_long)outP->alpha) / PF_MAX_CHAN8);
    ...
    for (A_long i = 0; i < w; i++) {
      ...
      if (p.alpha < PF_MAX_CHAN8) {
        if (p.alpha <= 0) {
          ...
          p.blue =
              RoundByteLong((A_long)p.blue * PF_MAX_CHAN8 / (A_long)p.alpha);
          p.green =
              RoundByteLong((A_long)p.green * PF_MAX_CHAN8 / (A_long)p.alpha);
...
PF_Err Exec08(CFsAE* ae, ParamInfo* infoP) {
  ...
  for (int i = 0; i < PCOUNT; i++) {
    ...
    switch (infoP->mode) {
      case MODE::Image:  // image
        ERR(ae->iterate8InputToTemp((refconType)&infoP->flareInfo[i],
                                    ImageBase));
        break;
      case MODE::Alpha:  // alpha
        ERR(ae->iterate8InputToTemp((refconType)&infoP->flareInfo[i],
    ...
    if ((infoP->flareInfo[i].rev != REVMODE::None) &&
        (infoP->mode != MODE::Image)) {
      ERR(RevRedChannel08(ae->tmp));
    }

    if ((infoP->flareInfo[i].border == TRUE) && (infoP->mode != MODE::Image) &&
        (infoP->flareInfo[i].rev == REVMODE::None)) {
    ...
    switch (infoP->flareInfo[i].blend) {
      case BLEND::Screen:
        ERR(ae->iterate8TempToOutput((refconType)&infoP->flareInfo[i],
                                     BlendScreen));
        break;
      case BLEND::Add:
        ERR(ae->iterate8TempToOutput((refconType)&infoP->flareInfo[i],
