...
static PF_Err BaseCopyWhiteBase(refconType refcon, A_long xL, A_long yL,
  ...
  if (a >= 1) {
    ...
    outP->red =
        RoundShortFpLong(v * (double)infoP->BaseColor.red / PF_MAX_CHAN16);
    outP->green =
        RoundShortFpLong(v * (double)infoP->BaseColor.green / PF_MAX_CHAN16);
    outP->blue =
        RoundShortFpLong(v * (double)infoP->BaseColor.blue / PF_MAX_CHAN16);
...
static PF_Err BaseCopyAlphaBase(refconType refcon, A_long xL, A_long yL,
  ...
  if (inP->alpha <= 0) {
    ...
    *outP = r;
    ...
    PF_Pixel16 r2 = {PF_MAX_CHAN16, PF_MAX_CHAN16, PF_MAX_CHAN16,
                     PF_MAX_CHAN16};
...
static PF_Err WhiteBase(refconType refcon, A_long xL, A_long yL,
                        PF_Pixel16* inP, PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err BlackBase(refconType refcon, A_long xL, A_long yL,
                        PF_Pixel16* inP, PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  FlareInfo16* infoP = reinterpret_cast<FlareInfo16*>(refcon);
...
static PF_Err AlphaBase(refconType refcon, A_long xL, A_long yL,
                        PF_Pixel16* inP, PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  FlareInfo16* infoP = reinterpret_cast<FlareInfo16*>(refcon);
...
static PF_Err ImageBase(refconType refcon, A_long xL, A_long yL,
                        PF_Pixel16* inP, PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  FlareInfo16* infoP = reinterpret_cast<FlareInfo16*>(refcon);
...
static PF_Err BlendScreen(refconType refcon, A_long xL, A_long yL,
  ...
  FlareInfo16* infoP = reinterpret_cast<FlareInfo16*>(refcon);
  outP->red =
      RoundShort((A_long)inP->red + (A_long)outP->red -
                 ((A_long)inP->red * (A_long)outP->red) / PF_MAX_CHAN16);
  outP->green =
      RoundShort((A_long)inP->green + (A_long)outP->green -
                 ((A_long)inP->green * (A_long)outP->green) / PF_MAX_CHAN16);
  outP->blue =
      RoundShort((A_long)inP->blue + (A_long)outP->blue -
                 ((A_long)inP->blue * (A_long)outP->blue) / PF_MAX_CHAN16);
  outP->alpha =
      RoundShort((A_long)inP->alpha + (A_long)outP->alpha -
                 ((A_long)inP->alpha * (A_long)outP->alpha) / PF_MAX_CHAN16);
    ...
    for (A_long i = 0; i < w; i++) {
      ...
      if (p.alpha < PF_MAX_CHAN16) {
        if (p.alpha <= 0) {
          ...
          p.green =
              RoundShort((A_long)p.green * PF_MAX_CHAN16 / (A_long)p.alpha);
...
PF_Err Exec16(CFsAE* ae, ParamInfo16* infoP) {
  ...
  for (int i = 0; i < PCOUNT; i++) {
    ...
    switch (infoP->mode) {
      case MODE::Image:  // image
        ERR(ae->iterate16InputToTemp((refconType)&infoP->flareInfo[i],
                                     ImageBase));
        break;
      case MODE::Alpha:  // alpha
        ERR(ae->iterate16InputToTemp((refconType)&infoP->flareInfo[i],
    ...
    switch (infoP->flareInfo[i].blend) {
      case BLEND::Screen:
        ERR(ae->iterate16TempToOutput((refconType)&infoP->flareInfo[i],
                                      BlendScreen));
        break;
      case BLEND::Add:
        ERR(ae->iterate16TempToOutput((refconType)&infoP->flareInfo[i],
