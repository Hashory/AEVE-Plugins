...
// ********************************************************************************************************************
// Passes parameters to After Effects.
...
static PF_Err ParamsSetup(PF_InData* in_data, PF_OutData* out_data,
  ...
  PF_ADD_POPUP(STR_MODE1,
               STR_MODE1_COUNT,  // number of menu items
               STR_MODE1_DEFL,   // default
               STR_MODE1_ITEMS, ID_MODE);
  // baseOn
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_BASE_ON, STR_ON, TRUE, 0, ID_BASE_ON);
  // baseColor
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_BASE_COLOR, 0xF4, 0xFF, 0x95, ID_BASE_COLOR);
  // baseOpacity
  AEFX_CLR_STRUCT(def);
...
#pragma region enabled
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_CHECKBOX(num, STR_ON, FI_Enabled[i], 0, PIDX(i) + ID_ENABLED);
#pragma endregion
...
#pragma region rev
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_POPUP(num,
                 STR_REV_COUNT,  // number of menu items
                 FI_REV[i],      // default
                 STR_REV_ITEMS, PIDX(i) + ID_REV);
#pragma endregion
...
#pragma region border
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_CHECKBOX(num, STR_ON, FI_BORDER[i], 0, PIDX(i) + ID_BORDER);
#pragma endregion
...
#pragma region inside
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_SLIDER(num,           // Parameter name
                  0,             // Minimum value for numeric input
                  20,            // Maximum value for numeric input
                  0,             // Minimum value for slider
                  10,            // Maximum value for slider
                  FI_INSIDE[i],  // Default value
                  PIDX(i) + ID_INSIDE);
#pragma endregion
...
#pragma region outside
    ...
    PF_ADD_SLIDER(num,            // Parameter name
                  ...
                  20,             // Maximum value for numeric input
                  0,              // Minimum value for slider
                  20,             // Maximum value for slider
                  FI_OUTSIDE[i],  // Default value
                  PIDX(i) + ID_OUTSIDE);
#pragma endregion
...
#pragma region color
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_COLOR(num, FI_COLOR[i].red, FI_COLOR[i].green, FI_COLOR[i].blue,
                 PIDX(i) + ID_COLOR);
#pragma endregion
...
#pragma region blend
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_POPUP(num,
                 STR_BLEND_COUNT,  // number of menu items
                 FI_BLEND[i],      // default
                 STR_BLEND_ITEMS, PIDX(i) + ID_BLEND);
#pragma endregion
...
static PF_Err GetParams(CFsAE* ae, ParamInfo* infoP) {
  ...
  if (infoP->BaseOpacity < 0)
    infoP->BaseOpacity = 0;
  else if (infoP->BaseOpacity > 1)
    infoP->BaseOpacity = 1;
  ...
  for (int i = 0; i < PCOUNT; i++) {
    ...
    infoP->flareInfo[i].inside =
        ae->downScaleNoClip(infoP->flareInfo[i].inside);
    ERR(ae->GetADD(PIDX(i) + ID_OUTSIDE, &infoP->flareInfo[i].outside));
    infoP->flareInfo[i].outside =
        ae->downScaleNoClip(infoP->flareInfo[i].outside);
    ...
    if (infoP->flareInfo[i].opacitry < 0)
      infoP->flareInfo[i].opacitry = 0;
    else if (infoP->flareInfo[i].opacitry > 1)
      infoP->flareInfo[i].opacitry = 1;
  }
...
(before After Effects 7). Writing this function makes it compatible with v6.5.
*/
