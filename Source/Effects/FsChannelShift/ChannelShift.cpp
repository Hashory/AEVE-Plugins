...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_ANGLE(STR_GREEN_ROT, 90.0, ID_GREEN_ROT);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_ANGLE(STR_BLUE_ROT, 180.0, ID_BLUE_ROT);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_ANGLE(STR_ALPHA_ROT, 270.0, ID_ALPHA_ROT);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_POPUP(STR_EDGE_POP1, 4,  // Number of menus
               1,                 // Default
               STR_EDGE_POP2, ID_EDGE_POP);
...
static void PramSet(CFsAE *ae, ParamInfo *infoP) {
  // Divide the movement distance into X and Y
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if ((infoP->r_length == 0) && (infoP->g_length == 0) &&
      (infoP->b_length == 0) && (infoP->a_length == 0)) {
    ERR(ae->CopyInToOut());
  } else {
    ERR(ae->out->clear());
