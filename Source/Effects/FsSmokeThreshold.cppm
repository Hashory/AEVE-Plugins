...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_HI_E, STR_ON, TRUE, 0, ID_HI_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_SDW1_E, STR_ON, TRUE, 0, ID_SDW1_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_SDW2_E, STR_ON, TRUE, 0, ID_SDW2_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_HI_COL, 0xF0, 0xF0, 0xF0, ID_HI_COL);

  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_NOR_COL, 0x90, 0x90, 0x90, ID_NOR_COL);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SDW1_COL, 0x50, 0x50, 0x50, ID_SDW1_COL);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SDW2_COL, 0x30, 0x30, 0x30, ID_SDW2_COL);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_NOR_LINE_E, STR_ON, TRUE, 0, ID_NOR_LINE_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_SDW1_LINE_E, STR_ON, TRUE, 0, ID_SDW1_LINE_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_SDW2_LINE_E, STR_ON, TRUE, 0, ID_SDW2_LINE_ENABLED);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_NOR_LINE, 0x50, 0x50, 0x50, ID_NOR_LINE);
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SDW1_LINE_E, 0x30, 0x30, 0x30, ID_SDW1_LINE);

  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SDW2_LINE_E, 0x20, 0x20, 0x20, ID_SDW2_LINE);
...
//-------------------------------------------------------------------------------------------------
static PF_Err alpha8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                     PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);

  A_long v =
      (A_long)((0.29891 * (double)outP->red) + (0.58661 * (double)outP->green) +
               (0.11448 * (double)outP->blue) + 0.5);

  if (outP->alpha >= niP->alpha) {
    outP->red = outP->blue = outP->alpha = PF_MAX_CHAN8;
  } else {
    outP->red = outP->blue = outP->alpha = 0;
  }
  if (outP->alpha == 0) {
    outP->green = 0;
  } else {
...
//-------------------------------------------------------------------------------------------------
static PF_Err alpha16(refconType refcon, A_long xL, A_long yL, PF_Pixel16 *inP,
                      PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ...
  if (outP->alpha >= niP->alpha) {
    outP->red = outP->blue = outP->alpha = PF_MAX_CHAN16;
  } else {
    outP->red = outP->blue = outP->alpha = 0;
  }
  if (outP->alpha == 0) {
...
//-------------------------------------------------------------------------------------------------
static PF_Err alpha32(refconType refcon, A_long xL, A_long yL,
                      PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo32 *niP = reinterpret_cast<ParamInfo32 *>(refcon);

  PF_FpShort v = (PF_FpShort)((0.29891 * outP->red) + (0.58661 * outP->green) +
                              (0.11448 * outP->blue));

  if (outP->alpha >= niP->alpha) {
    outP->red = outP->blue = outP->alpha = 1.0;
  } else {
    outP->red = outP->blue = outP->alpha = 0;
  }
  if (outP->alpha == 0) {
...
//-------------------------------------------------------------------------------------------------
static PF_Err outline8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                       PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
//-------------------------------------------------------------------------------------------------
static PF_Err outline16(refconType refcon, A_long xL, A_long yL,
...
//-------------------------------------------------------------------------------------------------
static PF_Err outline32(refconType refcon, A_long xL, A_long yL,
                        PF_PixelFloat *inP, PF_PixelFloat *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo32 *niP = reinterpret_cast<ParamInfo32 *>(refcon);
