...
static PF_Err ParamsSetup(PF_InData* in_data, PF_OutData* out_data,
  ...
  PF_ADD_FLOAT_SLIDER(STR_L_LV,  // Name
                      ...
                      ID_L_LV);
  AEFX_CLR_STRUCT(def);
  // def.flags 	= 	PF_ParamFlag_START_COLLAPSED;	//If this is set, it
  // will be expanded by default.
  PF_ADD_TOPIC(STR_TOPIC_NAIN_COL, ID_TOPIC_NAIN_COL);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_MAIN_COL1_ON, STR_ON, TRUE, 0, ID_MAIN_COL1_ON);
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // If this is set, keyframing is disabled
  PF_ADD_COLOR(STR_MAIN_COL1, 10, 10, 10, ID_MAIN_COL1);
  AEFX_CLR_STRUCT(def);
  ...
  PF_ADD_FLOAT_SLIDER(STR_MAIN_COL1_LV,  // Name
                      ...
                      ID_MAIN_COL1_LV);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_MAIN_COL2_ON, STR_ON, FALSE, 0, ID_MAIN_COL2_ON);
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // If this is set, keyframing is disabled
  PF_ADD_COLOR(STR_MAIN_COL2, PF_MAX_CHAN8 - 1, PF_MAX_CHAN8 - 1,
               PF_MAX_CHAN8 - 1, ID_MAIN_COL2);
  AEFX_CLR_STRUCT(def);
  ...
  PF_ADD_FLOAT_SLIDER(STR_MAIN_COL2_LV,  // Name
                      ...
                      ID_MAIN_COL2_LV);
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_MAIN_COL3_ON, STR_ON, FALSE, 0, ID_MAIN_COL3_ON);
  AEFX_CLR_STRUCT(def);
  def.flags =
      PF_ParamFlag_CANNOT_TIME_VARY;  // If this is set, keyframing is disabled
  PF_ADD_COLOR(STR_MAIN_COL3, 0, 0, 0, ID_MAIN_COL3);
  AEFX_CLR_STRUCT(def);
  ...
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER(STR_SUB_COL_COUNT,  // Parameter name
                0,                  // Minimum value for numeric input
                COUNT_OF_SUB,       // Maximum value for numeric input
                0,                  // Minimum value of the slider
                COUNT_OF_SUB,       // Maximum value of the slider
                0,                  // Default value
                ID_SUB_COL_COUNT);
  char num[255] = {"\0"};
...
#endif
    AEFX_CLR_STRUCT(def);
    def.flags = PF_ParamFlag_CANNOT_TIME_VARY;  // If this is set, keyframing is
                                                // disabled
    PF_ADD_COLOR(num, 0, 0, 0, ID_SUB_COL(i));
  }
  AEFX_CLR_STRUCT(def);
...
static PF_Err GetParams(CFsAE* ae, ParamInfo* infoP) {
  ...
  infoP->alpha_border =
      RoundByteFpLong((double)PF_MAX_CHAN8 * infoP->alpha_level + 0.5);
  ...
  if (!err) {
    if (infoP->sub_col_count > 0) {
      if (infoP->sub_col_count > COUNT_OF_SUB)
        infoP->sub_col_count = COUNT_OF_SUB;
      idx = 0;
...
/*
    Hosts that do not support SmartFX (before After Effects 7) call this
   function to render. Writing this function makes it compatible with v6.5.
*/
