...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_TARGET, 0xFF, 0x00, 0x00, ID_TARGET_COL);
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_SAMPLE, 0x00, 0xFF, 0x00, ID_SAMPLE_COL);
  ...
  // Color specification
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_DRAW, 0x00, 0x00, 0xFF, ID_DRAW_COL);
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  for (int y = y0; y <= y1; y++) {
    for (int x = x0; x <= x1; x++) {
      if (compPix8Lv(niP->ae->in->getPixel8(x, y), niP->sampleCol,
                     niP->level) == TRUE) {
        *outP = niP->drawCol;
        break;
      }
...
static PF_Err FilterImage32(refconType refcon, A_long xL, A_long yL,
  ...
  for (int y = y0; y <= y1; y++) {
    for (int x = x0; x <= x1; x++) {
      if (compPix32_8Lv(niP->ae->in->getPixel32(x, y), niP->sampleCol,
                        niP->level) == TRUE) {
        *outP = niP->drawCol;
        break;
      }
...
/*
        Hosts that do not support SmartFX (After Effects 7 and earlier) call
   this function for rendering Writing this function provides basic v6.5
   compatibility
*/
