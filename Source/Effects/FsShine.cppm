...
PF_Err CopyIntoBuf1_16(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    for (A_long x = 0; x < w; x++) {
      ...
      PF_FpLong vmax = (PF_FpLong)p[0].red / PF_MAX_CHAN16 +
                       (PF_FpLong)p[0].green / PF_MAX_CHAN16 +
                       (PF_FpLong)p[0].blue / PF_MAX_CHAN16;
      A_long idx = 0;
      for (A_long i = 1; i < 4; i++) {
        PF_FpLong vv = (PF_FpLong)p[i].red / PF_MAX_CHAN16 +
                       (PF_FpLong)p[i].green / PF_MAX_CHAN16 +
                       (PF_FpLong)p[i].blue / PF_MAX_CHAN16;
        if (vv > vmax) {
          idx = i;
          vmax = vv;
        }
...
PF_Err CopyIntoBuf1_32(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    for (A_long x = 0; x < w; x++) {
      ...
      PF_FpLong vmax =
          (PF_FpLong)p[0].red + (PF_FpLong)p[0].green + (PF_FpLong)p[0].blue;
      A_long idx = 0;
      for (A_long i = 1; i < 4; i++) {
        PF_FpLong vv =
            (PF_FpLong)p[i].red + (PF_FpLong)p[i].green + (PF_FpLong)p[i].blue;
        if (vv > vmax) {
          idx = i;
          vmax = vv;
        }
...
PF_Err CopyIntoBuf1G_16(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    for (A_long x = 0; x < w; x++) {
      ...
      PF_Pixel16 c = {PF_MAX_CHAN16, PF_MAX_CHAN16, PF_MAX_CHAN16,
                      PF_MAX_CHAN16};
...
PF_Err CopyBuf1ToDst_8(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    for (A_long x = 0; x < w; x++) {
      ...
      infoP->outP->SetPix8(x, y, infoP->bufP1->GetPixD8(xx, yy));
    }
  }
  return err;
}
PF_Err CopyBuf1ToDst_16(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    for (A_long x = 0; x < w; x++) {
      ...
      infoP->outP->SetPix16(x, y, infoP->bufP1->GetPixD16(xx, yy));
    }
  }
  return err;
}
PF_Err CopyBuf1ToDst_32(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    for (A_long x = 0; x < w; x++) {
      ...
      infoP->outP->SetPix32(x, y, infoP->bufP1->GetPixD32(xx, yy));
    }
  }
  return err;
}
...
PF_Err CopyBuf2ToDst_8(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    for (A_long x = 0; x < w; x++) {
      ...
      infoP->outP->SetPix8(x, y, infoP->bufP2->GetPixD8(xx, yy));
    }
  }
  return err;
}
PF_Err CopyBuf2ToDst_16(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    for (A_long x = 0; x < w; x++) {
      ...
      infoP->outP->SetPix16(x, y, infoP->bufP2->GetPixD16(xx, yy));
    }
  }
  return err;
}
PF_Err CopyBuf2ToDst_32(ParamInfo* infoP) {
  ...
  for (A_long y = 0; y < h; y++) {
    ...
    for (A_long x = 0; x < w; x++) {
      ...
      infoP->outP->SetPix32(x, y, infoP->bufP2->GetPixD32(xx, yy));
    }
  }
  return err;
}
...
static PF_Err sub8(void* refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                   PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ...
  // lenDターゲットからの距離
  PF_FpLong lenD =
      pointLength(infoP->outP->in_data, (PF_FpLong)xL, (PF_FpLong)yL, cx, cy);
  // len ターゲットからの距離 long
...
//-------------------------------------------------------------------------------------------------
static PF_Err sub16(void* refcon, A_long xL, A_long yL, PF_Pixel16* inP,
                    PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
  A_long vlen = (A_long)((double)infoP->length / 2 + 0.5);
...
//-------------------------------------------------------------------------------------------------
static PF_Err sub32(void* refcon, A_long xL, A_long yL, PF_PixelFloat* inP,
                    PF_PixelFloat* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
  A_long vlen = (A_long)((double)infoP->length / 2 + 0.5);
...
static PF_Err subG8(void* refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                    PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
static PF_Err subG16(void* refcon, A_long xL, A_long yL, PF_Pixel16* inP,
                     PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
static PF_Err subG32(void* refcon, A_long xL, A_long yL, PF_PixelFloat* inP,
                     PF_PixelFloat* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
  A_long vlen = (A_long)((PF_FpLong)infoP->length / 2 + 0.5);
...
#pragma endregion
static PF_Err subScr8(void* refcon, A_long xL, A_long yL, PF_Pixel8* inP,
                      PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
static PF_Err subScr16(void* refcon, A_long xL, A_long yL, PF_Pixel16* inP,
                       PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
static PF_Err subScr32(void* refcon, A_long xL, A_long yL, PF_PixelFloat* inP,
                       PF_PixelFloat* outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo* infoP = reinterpret_cast<ParamInfo*>(refcon);
...
PF_Err Shine::ParamsSetup(PF_InData* in_dataP, PF_OutData* out_dataP,
                          PF_ParamDef* paramsP[], PF_LayerDef* outputP) {
  PF_Err err = PF_Err_NONE;
  Init();
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_ISCOLOR, STR_ON, FALSE, 0, ID_ISCOLOR);
  //----------------------------------------------------------------
  // 色の指定
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(STR_COLOR, 0xFF, 0xFF, 0xFF, ID_COLOR);
  //----------------------------------------------------------------
...
PF_Err Shine::Exec(ParamInfo* infoP) {
  ...
  if ((infoP->length <= 0) || (infoP->strong <= 0)) {
    ...
    PF_EffectWorld buf1 = NewEffectWorld(
        infoP->outP->width() / 2, infoP->outP->height() / 2, pixelFormat());
    PF_EffectWorld buf2 = NewEffectWorld(
        infoP->outP->width() / 2, infoP->outP->height() / 2, pixelFormat());
    ...
    switch (pixelFormat()) {
      case PF_PixelFormat_ARGB128:
        if (infoP->isColor) {
          ...
          iterate32(infoP->bufP1->world, (void*)infoP, subG32,
                    infoP->bufP2->world);
          CopyBuf2ToDst_32(infoP);
        } else {
          CopyIntoBuf1_32(infoP);
          iterate32(infoP->bufP1->world, (void*)infoP, sub32,
                    infoP->bufP2->world);
          CopyBuf2ToDst_32(infoP);
          infoP->outP->FromMat();
        }
      ...
      case PF_PixelFormat_ARGB64:
        if (infoP->isColor) {
          ...
          iterate16(infoP->bufP1->world, (void*)infoP, subG16,
                    infoP->bufP2->world);
          CopyBuf2ToDst_16(infoP);
        } else {
          CopyIntoBuf1_16(infoP);
          iterate16(infoP->bufP1->world, (void*)infoP, sub16,
                    infoP->bufP2->world);
          CopyBuf2ToDst_16(infoP);
          infoP->outP->FromMat();
        }
      ...
      case PF_PixelFormat_ARGB32:
        if (infoP->isColor) {
          ...
          iterate8(infoP->bufP1->world, (void*)infoP, subG8,
                   infoP->bufP2->world);
          CopyBuf2ToDst_8(infoP);
        } else {
          CopyIntoBuf1_8(infoP);
          iterate8(infoP->bufP1->world, (void*)infoP, sub8,
                   infoP->bufP2->world);
          CopyBuf2ToDst_8(infoP);
