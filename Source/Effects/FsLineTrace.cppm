...
static PF_Pixel16 WHT16 = {PF_MAX_CHAN16, PF_MAX_CHAN16, PF_MAX_CHAN16,
                           PF_MAX_CHAN16};
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //-----------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ENABLED_BLACK, UI_ENABLED2, TRUE, 0, enabled_black);
  ...
  PF_ADD_FIXED(UI_BORDER_BLACK,  // パラメータの名前
               ...
               0, 0, border_black);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(UI_DRAW_BLACK, 0x00, 0x00, 0x00, color_black);
  //-----------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ENABLED_RED, UI_ENABLED2, TRUE, 0, enabled_red);
  ...
  PF_ADD_FIXED(UI_BORDER_RED,  // パラメータの名前
               ...
               0, 0, border_red);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(UI_DRAW_RED, PF_MAX_CHAN8, 0x00, 0x00, color_red);

  //-----------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ENABLED_GREEN, UI_ENABLED2, TRUE, 0, enabled_green);
  ...
  PF_ADD_FIXED(UI_BORDER_GREEN,  // パラメータの名前
               ...
               0, 0, border_green);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(UI_DRAW_GREEN, 0x00, PF_MAX_CHAN8, 0x00, color_green);
  //-----------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ENABLED_BLUE, UI_ENABLED2, TRUE, 0, enabled_blue);
  ...
  PF_ADD_FIXED(UI_BORDER_BLUE,  // パラメータの名前
               ...
               0, 0, border_blue);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(UI_DRAW_BLUE, 0x00, 0x00, PF_MAX_CHAN8, color_blue);
  //-----------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ENABLED_YELLOW, UI_ENABLED2, TRUE, 0, enabled_yellow);
  ...
  PF_ADD_FIXED(UI_BORDER_YELLOW,  // パラメータの名前
               ...
               0, 0, border_yellow);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(UI_DRAW_YELLOW, PF_MAX_CHAN8, PF_MAX_CHAN8, 0x00, color_yellow);
  //-----------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(UI_ENABLED_VIOLET, UI_ENABLED2, TRUE, 0, enabled_violet);
  ...
  PF_ADD_FIXED(UI_BORDER_VIOLET,  // パラメータの名前
               ...
               0, 0, border_violet);
  AEFX_CLR_STRUCT(def);
  PF_ADD_COLOR(UI_DRAW_VIOLET, PF_MAX_CHAN8, 0x00, PF_MAX_CHAN8, color_violet);
  ...
  if (infoP) {
    ...
    if (op <= 0) {
      ...
      if (hls[HLS_L] >= TARGET_WHITE) {
        ...
        if ((infoP->ParamTable[LT_BLACK].border > hls[HLS_L]) &&
            (infoP->ParamTable[LT_BLACK].enabled == TRUE)) {
          p = BLK8;
        } else {
          p = WHT8;
        }
        ...
        if ((H >= RANGE_RED2) || ((H >= RANGE_RED0) && (H < RANGE_RED1))) {
          if ((infoP->ParamTable[LT_RED].border > hls[HLS_L]) &&
              (infoP->ParamTable[LT_RED].enabled == TRUE)) {
            p = infoP->ParamTable[LT_RED].color;
          } else {
            p = WHT8;
          }
        } else if ((H >= TARGET_YELLOW0) && (H < TARGET_YELLOW1)) {
          if ((infoP->ParamTable[LT_YELLOW].border > hls[HLS_L]) &&
              (infoP->ParamTable[LT_YELLOW].enabled == TRUE)) {
            p = infoP->ParamTable[LT_YELLOW].color;
          } else {
            p = WHT8;
          }
        } else if ((H >= TARGET_GREEN0) && (H < TARGET_GREEN1)) {
          if ((infoP->ParamTable[LT_GREEN].border > hls[HLS_L]) &&
              (infoP->ParamTable[LT_GREEN].enabled == TRUE)) {
            p = infoP->ParamTable[LT_GREEN].color;
          } else {
            p = WHT8;
          }
        } else if ((H >= TARGET_BLUE0) && (H < TARGET_BLUE1)) {
          if ((infoP->ParamTable[LT_BLUE].border > hls[HLS_L]) &&
              (infoP->ParamTable[LT_BLUE].enabled == TRUE)) {
          ...
          if ((infoP->ParamTable[LT_VIOLET].border > hls[HLS_L]) &&
              (infoP->ParamTable[LT_VIOLET].enabled == TRUE)) {
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  switch (ae->pixelFormat()) {
    case PF_PixelFormat_ARGB128:
      ...
      ERR(AEFX_AcquireSuite(ae->in_data, ae->out_data,
                            kPFColorCallbacksFloatSuite,
                            kPFColorCallbacksFloatSuiteVersion1,
                            "Couldn't load suite.", (void **)&(info32.cs)));
      if (!err) {
        if (info32.cs != NULL) {
          ERR(ae->iterate32((refconType)&info32, FilterImage32));

          ERR2(AEFX_ReleaseSuite(
              ae->in_data, ae->out_data, kPFColorCallbacksFloatSuite,
              kPFColorCallbacksFloatSuiteVersion1, "Couldn't release suite."));
        }
      }
      break;
    case PF_PixelFormat_ARGB64:
      ...
      ERR(AEFX_AcquireSuite(ae->in_data, ae->out_data, kPFColorCallbacks16Suite,
                            kPFColorCallbacks16SuiteVersion1,
                            "Couldn't load suite.", (void **)&(info16.cs)));
      if (!err) {
        if (info16.cs != NULL) {
          ERR(ae->iterate16((refconType)&info16, FilterImage16));

          ERR2(AEFX_ReleaseSuite(
              ae->in_data, ae->out_data, kPFColorCallbacks16Suite,
              kPFColorCallbacks16SuiteVersion1, "Couldn't release suite."));
        }
      }
      break;
    case PF_PixelFormat_ARGB32:
      ...
      ERR(AEFX_AcquireSuite(ae->in_data, ae->out_data, kPFColorCallbacksSuite,
                            kPFColorCallbacksSuiteVersion1,
                            "Couldn't load suite.", (void **)&(info8.cs)));
      if (!err) {
        if (info8.cs != NULL) {
          ERR(ae->iterate8((refconType)&info8, FilterImage8));

          ERR2(AEFX_ReleaseSuite(
              ae->in_data, ae->out_data, kPFColorCallbacksSuite,
              kPFColorCallbacksSuiteVersion1, "Couldn't release suite."));
        }
      }
      break;
  }
