...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (((infoP->value > 0) && (infoP->edge_value > 0)) && (infoP->opa > 0)) {
    A_long hsize1 =
        ae->out->width() * (ae->out->height() + 8) * sizeof(A_u_char);
    ...
    /*
    CFsBuffer buf1 = ae->NewBuffer( ae->out->width()
    *(ae->out->height()+8)*sizeof(A_u_char));// + SPD_RAND_MAX if
    (buf1.alive()==FALSE){ ae->out_data->out_flags |=
