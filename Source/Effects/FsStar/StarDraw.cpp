...
inline PF_Boolean pset8(CFsGraph *g, PF_Fixed x, PF_Fixed y, PF_Pixel p) {
  ...
  A_long w = g->width();
  A_long h = g->height();
  if ((x0 < -2) || (y0 < -2) || (x0 >= w) || (y0 >= h) || (x1 < 0) || (y1 < 0))
    return FALSE;
...
PF_Pixel calcColorTableOnly8(ColorTableInfo8 *ci) {
  ...
  if ((ci->length <= 0) || (ci->strong <= 0) || (ci->length <= ci->value))
    return ret;

  PF_FpLong op =
      ci->strong * (PF_FpLong)(ci->length - ci->value) / (PF_FpLong)ci->length;
...
PF_Pixel calcColorTable8(ColorTableInfo8 *ci) {
  ...
  if ((bd <= 0) || (ci->value >= bd)) {
    ...
    ret.green =
        RoundByteFpLong((ci->col1.green * d1 + ci->col2.green * d2) * op);
...
PF_Pixel16 calcColorTable16(ColorTableInfo16 *ci) {
  ...
  if ((bd <= 0) || (ci->value >= bd)) {
    ...
    ret.green =
        RoundShortFpLong((ci->col1.green * d1 + ci->col2.green * d2) * op);
...
PF_PixelFloat calcColorTable32(ColorTableInfo32 *ci) {
  ...
  if ((bd <= 0) || (ci->value >= bd)) {
    ...
    PF_FpLong d1 = 1 - d2;
    ret.blue =
        RoundFpShortDouble((ci->col1.blue * d1 + ci->col2.blue * d2) * op);
    ret.green =
        RoundFpShortDouble((ci->col1.green * d1 + ci->col2.green * d2) * op);
    ...
    for (x = 0; x < infoP->g->width(); x++) {
      ...
      if (base > 0) {
        for (A_long i = 0; i < 4; i++) {
          ...
          infoP->colTable.strong =
              infoP->line[i].strong * (PF_FpShort)base / PF_MAX_CHAN8;
    ...
    for (x = 0; x < infoP->g->width(); x++) {
      ...
      if (base > 0) {
        for (A_long i = 0; i < 4; i++) {
          ...
          infoP->colTable.strong =
              infoP->line[i].strong * (PF_FpShort)base / PF_MAX_CHAN16;
          if ((infoP->colTable.length > 0) && (infoP->colTable.strong > 0)) {
            makeTable16(infoP);
            DrawLine16(infoP);
          }
  ...
  for (y = 0; y < infoP->g->height(); y++) {
    ...
    for (x = 0; x < infoP->g->width(); x++) {
      ...
      if (base > 0) {
        ...
        for (A_long i = 0; i < 4; i++) {
          infoP->colTable.angle = infoP->line[i].angle;
          infoP->colTable.length =
              (A_long)(infoP->line[i].length * base / PF_MAX_CHAN16);
          infoP->colTable.strong =
              infoP->line[i].strong * (PF_FpShort)base / PF_MAX_CHAN16;
          if ((infoP->colTable.length > 0) && (infoP->colTable.strong > 0)) {
            makeTable16(infoP);
