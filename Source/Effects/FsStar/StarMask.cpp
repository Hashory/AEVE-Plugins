...
typedef struct MinParam {
  ...
  A_long scanlineLength;  // length of scanline
  A_long value;           // Max/Min value

  A_long target;  // target index of scanline
  A_long maxIndex;
} MinParam;
...
static PF_Err SubtrackAlpha8(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel8* inP, PF_Pixel8* outP) {
  PF_Err err = PF_Err_NONE;
  outP->alpha = RoundByteLong(outP->red - outP->alpha);
  return err;
}
static PF_Err SubtrackAlpha16(refconType refcon, A_long xL, A_long yL,
                              PF_Pixel16* inP, PF_Pixel16* outP) {
  PF_Err err = PF_Err_NONE;
  outP->alpha = RoundShort(outP->red - outP->alpha);
  return err;
}
static PF_Err SubtrackAlpha32(refconType refcon, A_long xL, A_long yL,
