...
// Notify After Effects of parameters
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_CHECKBOX("Vary noise every frame", "ON", TRUE, 0, ID_ANIMATED_CB);

  AEFX_CLR_STRUCT(def);
  def.ui_flags = PF_PUI_DISABLED;
  PF_ADD_SLIDER("Noise motion",   // Parameter name
                0,                // Minimum value for numeric input
                F_RAND_MAX,       // Maximum value for numeric input
                0,                // Minimum value for slider
                F_RAND_MAX / 10,  // Maximum value for slider
                0,                // Default value
                ID_ANIMATED_ADD);
  // Second parameter
  // Integer slider bar
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER("Amount of noise",  // Parameter name
                0,                  // Minimum value for numeric input
  ...
  // Checkbox
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX("Color noise", "ON", (PF_ParamValue)FALSE, 0, ID_COLOR_CB);
  ...
  PF_ADD_FIXED("Block amount",  // Parameter name
               ...
               0,               // Minimum value for slider
               5,               // Maximum value for slider
               1,               // Default value
               1,               // Flag for numeric display
               0, 0, ID_BLOCK_VALUE_FIXED);
  // Integer slider bar
  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED("Block strength",  // Parameter name
               ...
               5,                 // Default value
               1,                 // Flag for numeric display
               0, 0, ID_BLOCK_LENGTH_FIXED);
  // Integer slider bar
  AEFX_CLR_STRUCT(def);
  PF_ADD_SLIDER("Block width (px)",  // Parameter name
                3,                   // Minimum value for numeric input
                512,                 // Maximum value for numeric input
                3,                   // Minimum value for slider
                100,                 // Maximum value for slider
                16,                  // Default value
                ID_BLOCK_WIDTH_ADD);
  // Integer slider bar
  ...
  // Checkbox
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX("Color block", "ON", FALSE, 0, ID_BLOCK_COLOR_CB);
...
static PF_Err HandleChangedParam(PF_InData *in_data, PF_OutData *out_data,
  ...
  try {
    CFsAE ae;
    err = ae.HandleChangedParam(in_data, out_data, params, outputP, extraP,
                                ID_NUM_PARAMS);
    if (!err) {
      PF_Boolean b = FALSE;
      ...
      if (!err) {
        out_data->out_flags |=
            PF_OutFlag_FORCE_RERENDER | PF_OutFlag_REFRESH_UI;
      }
    }
  } catch (PF_Err &errP) {
    err = errP;
...
static PF_Err QueryDynamicFlags(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_Err err = PF_Err_NONE, err2 = PF_Err_NONE;
  // Set the value of PF_OutFlag_NON_PARAM_VARY to out_flags to switch whether
  // to draw every frame.
  CFsAE ae;
...
static void NoiseMain8(PrmTbl *prm) {
  ...
  ar = -1 * PF_MAX_CHAN8 +
       (A_long)(F_RAND() * (PF_MAX_CHAN8 * 2 + 1.0) / (1.0 + F_RAND_MAX));
  ar = ar * prm->length >> 16;
...
static void NoiseMain16(PrmTbl *prm) {
  ...
  ar = -1 * PF_MAX_CHAN16 +
       (A_long)(F_RAND() * (PF_MAX_CHAN16 * 2 + 1.0) / (1.0 + F_RAND_MAX));
  ar = ar * prm->length >> 16;
  if (prm->colorFlag == TRUE) {
    ag = -1 * PF_MAX_CHAN16 +
         (A_long)(F_RAND() * (PF_MAX_CHAN16 * 2 + 1.0) / (1.0 + F_RAND_MAX));
...
static void NoiseMain32(PrmTbl *prm) {
  ...
  ar = (PF_FpShort)(-1 + ((F_RAND() * 2.0 + 1.0) / (1.0 + F_RAND_MAX)));
  ar = (PF_FpShort)(ar * prm->length / 65536);
  if (prm->colorFlag == TRUE) {
    ag = (PF_FpShort)(-1 + ((F_RAND() * 2.0 + 1.0) / (1.0 + F_RAND_MAX)));
    ag = (PF_FpShort)(ag * prm->length / 65536);
    ab = (PF_FpShort)(-1 + ((F_RAND() * 2.0 + 1.0) / (1.0 + F_RAND_MAX)));
    ab = (PF_FpShort)(ab * prm->length / 65536);
  } else {
    ag = ab = ar;
    F_RAND();
    F_RAND();
...
static void NoiseBlock8(PrmTbl *prm) {
  ...
  ar = PF_MAX_CHAN8 - F_RAND() * PF_MAX_CHAN8 * 2 / F_RAND_MAX;
  ar = ar * prm->block_length >> 16;
  if (prm->block_colorFlag == TRUE) {
    ag = PF_MAX_CHAN8 - F_RAND() * PF_MAX_CHAN8 * 2 / F_RAND_MAX;
    ag = ag * prm->block_length >> 16;
    ab = PF_MAX_CHAN8 - F_RAND() * PF_MAX_CHAN8 * 2 / F_RAND_MAX;
    ab = ab * prm->block_length >> 16;
  } else {
    ag = ab = ar;
    F_RAND();
    F_RAND();
...
static void NoiseBlock16(PrmTbl *prm) {
  ...
  ar = PF_MAX_CHAN16 - F_RAND() * PF_MAX_CHAN16 * 2 / F_RAND_MAX;
  ar = ar * prm->block_length >> 16;
  if (prm->block_colorFlag == TRUE) {
    ag = PF_MAX_CHAN16 - F_RAND() * PF_MAX_CHAN16 * 2 / F_RAND_MAX;
    ag = ag * prm->block_length >> 16;
    ab = PF_MAX_CHAN16 - F_RAND() * PF_MAX_CHAN16 * 2 / F_RAND_MAX;
    ab = ab * prm->block_length >> 16;
  } else {
    ag = ab = ar;
    F_RAND();
...
static void NoiseBlock32(PrmTbl *prm) {
  ...
  ar = (PF_FpShort)(1 - ((PF_FpShort)F_RAND() * 2 / F_RAND_MAX));
  ar = (PF_FpShort)(ar * prm->block_length / 65536);
  if (prm->block_colorFlag == TRUE) {
    ag = (PF_FpShort)(1 - (PF_FpShort)F_RAND() * 2 / F_RAND_MAX);
    ag = (PF_FpShort)(ag * prm->block_length / 65536);
    ab = (PF_FpShort)(1 - (PF_FpShort)F_RAND() * 2 / F_RAND_MAX);
    ab = (PF_FpShort)(ab * prm->block_length / 65536);
  } else {
    ag = ab = ar;
    F_RAND();
    F_RAND();
...
static PF_Err Exec(CFsAE *ae, PrmTbl *infoP) {
  PF_Err err = PF_Err_NONE;
