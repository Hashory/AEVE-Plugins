...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_ADD_FIXED(Str_RED_VALUE,  // Parameter name
               0,              // Min value for numeric input
               300,            // Max value for numeric input
               0,              // Min value for slider
               100,            // Max value for slider
               0,              // Default value
               1,              // Flags for numeric display
               0, 0, ID_RED_NOISE);

  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(Str_RED_OPT, 0, 200, 0, 100, 100, 1, 0, 0, ID_RED_OPT);

  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(Str_GREEN_VALUE, 0, 300, 0, 100, 0, 1, 0, 0, ID_GREEN_NOISE);

  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(Str_GREEN_OPT, 0, 200, 0, 100, 100, 1, 0, 0, ID_GREEN_OPT);

  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(Str_BLUE_VALUE, 0, 300, 0, 100, 0, 1, 0, 0, ID_BLUE_NOISE);

  AEFX_CLR_STRUCT(def);
  PF_ADD_FIXED(Str_BLUE_OPT, 0, 200, 0, 100, 100, 1, 0, 0, ID_BLUE_OPT);
...
static PF_Err QueryDynamicFlags(PF_InData *in_data, PF_OutData *out_data,
  ...
  PF_Err err = PF_Err_NONE, err2 = PF_Err_NONE;
  // Set PF_OutFlag_NON_PARAM_VARY to out_flags to switch between rendering
  // every frame.
  return err;
}

inline A_long randValue(A_long v) { return F_RAND2(-v, v); }
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  if (infoP) {
    ...
    if (outP->alpha > 0) {
      ...
      // To keep the random number sequence consistent
      F_RAND();
      F_RAND();
      F_RAND();
    }
