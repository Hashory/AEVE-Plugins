...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data,
  ...
  //----------------------------------------------------------------
  AEFX_CLR_STRUCT(def);
  PF_ADD_CHECKBOX(STR_DRAW_COL, STR_ON, FALSE, 0, ID_DRAW_COL);
...
static PF_Err FilterImage8(refconType refcon, A_long xL, A_long yL,
  ...
  double v =
      ((0.29891 * (PF_FpLong)inP->red) + (0.58661 * (PF_FpLong)inP->green) +
...
static PF_Err FilterImage16(refconType refcon, A_long xL, A_long yL,
  ...
  double v = ((0.29891 * (PF_FpLong)c.red) + (0.58661 * (PF_FpLong)c.green) +
              (0.11448 * (PF_FpLong)c.blue));
...
//-------------------------------------------------------------------------------------------------
static PF_Err DrawCol8(CFsAE *ae, ParamInfo *infoP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err DrawCol16(CFsAE *ae, ParamInfo *infoP) {
  PF_Err err = PF_Err_NONE;
  // if (infoP->drawCol == FALSE) return err;
...
static PF_Err DrawCol32(CFsAE *ae, ParamInfo *infoP) {
  PF_Err err = PF_Err_NONE;
  // if (infoP->drawCol == FALSE) return err;
