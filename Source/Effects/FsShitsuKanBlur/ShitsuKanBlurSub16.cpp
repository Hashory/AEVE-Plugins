...
// Gaussian table for blurring
static PF_Boolean makeTbl16(ParamInfo16 *infoP) {
  PF_InData *in_data;
  in_data = infoP->in_data;
  A_long range;
  if (infoP->IsTagetHi)
    range = infoP->info.hi_blur;
  else
    range = infoP->info.sw_blur;
...
// Horizontal blur of only Green
void horBlurG16(ParamInfo16 *infoP) {
  A_long blur = 0;
  if (infoP->IsTagetHi)
    blur = infoP->info.hi_blur;
  else
    blur = infoP->info.sw_blur;
  if (blur <= 0)
    return;
...
static PF_Pixel16 GetPX16_CLIP(ParamInfo16 *infoP, A_long x, A_long y) {
  ...
  if (xx < 0)
    xx = 0;
  else if (xx > infoP->width2)
    xx = infoP->width2;

  if (yy < 0)
    yy = 0;
  else if (yy > infoP->height2)
    yy = infoP->height2;
...
static PF_Err Iterate16(CFsAE *ae, ParamInfo16 *niP,
                        PF_Err (*pix_fn)(refconType refcon, A_long x, A_long y,
...
static PF_Err GetTargetPixels16(refconType refcon, A_long xL, A_long yL,
...
static PF_Err AlphaTargetToGreen16(refconType refcon, A_long xL, A_long yL,
                                   PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err SwapGB16(refconType refcon, A_long xL, A_long yL, PF_Pixel16 *inP,
                       PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err RedToGreen16(refconType refcon, A_long xL, A_long yL,
                           PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err AlphaON16(refconType refcon, A_long xL, A_long yL,
                        PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err MaskedGBFromA16(refconType refcon, A_long xL, A_long yL,
                              PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err MaxHorG2R16(refconType refcon, A_long xL, A_long yL,
                          PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
...
static PF_Err MaxVurG2R16(refconType refcon, A_long xL, A_long yL,
                          PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
...
static PF_Err MoveG2R16(refconType refcon, A_long xL, A_long yL,
                        PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
  ...
  // Since PF_MAX_CHAN16 = 32768 is 0b10000000_00000000
  A_u_short r = 0;
  if (outP->green > 0)
    r |= 0x7F00;
  if (p1.green > 0)
...
static PF_Err SplitG2RGB16(refconType refcon, A_long xL, A_long yL,
                           PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
...
static PF_Err Noise16(refconType refcon, A_long xL, A_long yL, PF_Pixel16 *inP,
                      PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
...
static PF_Err CopyTagetEX16(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
  ...
  if (outP->alpha != PF_MAX_CHAN16) {
    ...
    if (niP->info.white == TRUE) {
      if ((inP->red == PF_MAX_CHAN16) && (inP->green == PF_MAX_CHAN16) &&
          (inP->blue == PF_MAX_CHAN16)) {
        b = FALSE;
      }
    }
    if (b == TRUE) {
...
static PF_Err GreenToAlpha16(refconType refcon, A_long xL, A_long yL,
                             PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
...
static PF_Err BlueToAlpha16(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
...
static PF_FpLong LvSet16(PF_FpLong src, PF_FpLong lv, PF_FpLong alpha) {
  PF_FpLong ret = 0;
...
static PF_Err ColorSet16(refconType refcon, A_long xL, A_long yL,
                         PF_Pixel16 *inP, PF_Pixel16 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo16 *niP = reinterpret_cast<ParamInfo16 *>(refcon);
...
static PF_Err debug16(CFsAE *ae, ParamInfo16 *pi) {
  PF_Err err = PF_Err_NONE;
...
PF_Err ToParam16(CFsAE *ae, ParamInfo *infoP, ParamInfo16 *pi) {
  ...
  for (int i = 0; i < pi->info.Color_Max; i++)
    pi->info.Color[i] = infoP->Color[i];
  for (int i = 0; i < pi->info.EXColor_Max; i++)
    pi->info.EXColor[i] = infoP->EXColor[i];
  ...
  if (pi->scanlineSize < pi->height)
    pi->scanlineSize = pi->height;
  ...
  if (pi->blurSize < pi->info.sw_blur)
    pi->blurSize = pi->info.sw_blur;
  ...
  if (pi->info.hi_length > 0) {
    ...
    if (pi->hi_x_offset > 0)
      pi->hi_x_offset += LM;
    else if (pi->hi_x_offset < 0)
      pi->hi_x_offset -= LM;
    if (pi->hi_y_offset > 0)
      pi->hi_y_offset += LM;
    else if (pi->hi_y_offset < 0)
      pi->hi_y_offset -= LM;
  } else {
    pi->hi_x_offset = 0;
    pi->hi_y_offset = 0;
  ...
  if (pi->info.sw_length > 0) {
    ...
    if (pi->sw_x_offset > 0)
      pi->sw_x_offset += LM;
    else if (pi->sw_x_offset < 0)
      pi->sw_x_offset -= LM;
    if (pi->sw_y_offset > 0)
      pi->sw_y_offset += LM;
    else if (pi->sw_y_offset < 0)
      pi->sw_y_offset -= LM;
  } else {
    pi->sw_x_offset = 0;
    pi->sw_y_offset = 0;
  ...
  if (pi.TargetCount > 0) {
    ...
    if ((pi.info.hi_length > 0) || (pi.info.sw_length > 0)) {
      ...
      if ((pi.info.hi_blur > 0) || (pi.info.sw_blur > 0)) {
        pi.scanlineH = ae->NewHandle(pi.scanlineSize * sizeof(A_u_short) * 4 +
                                     pi.blurSize * sizeof(A_long) + 100);
...
First, create a mask of the Target area in Alpha
