...
// Horizontal blur only for Green
...
static PF_Err Iterate(CFsAE *ae, ParamInfo8 *niP,
...
static PF_Err GetTargetPixels8(refconType refcon, A_long xL, A_long yL,
...
static PF_Err AlphaTargetToGreen(refconType refcon, A_long xL, A_long yL,
                                 PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err SwapGB(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                     PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err RedToGreen(refconType refcon, A_long xL, A_long yL,
                         PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err MaskedGBFromA(refconType refcon, A_long xL, A_long yL,
                            PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
...
static PF_Err MaxHorG2R(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                        PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_Err MaxVurG2R(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                        PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_Err MoveG2R(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                      PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_Err SplitG2RGB(refconType refcon, A_long xL, A_long yL,
                         PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_Err Noise8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                     PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_Err CopyTagetEX(refconType refcon, A_long xL, A_long yL,
                          PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
  ...
  if (outP->alpha != PF_MAX_CHAN8) {
    ...
    if (niP->info.white == TRUE) {
      if ((inP->red == PF_MAX_CHAN8) && (inP->green == PF_MAX_CHAN8) &&
          (inP->blue == PF_MAX_CHAN8)) {
        b = FALSE;
      }
    }
    if (b == TRUE) {
...
static PF_Err GreenToAlpha(refconType refcon, A_long xL, A_long yL,
                           PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_Err BlueToAlpha(refconType refcon, A_long xL, A_long yL,
                          PF_Pixel8 *inP, PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_FpLong LvSet(PF_FpLong src, PF_FpLong lv, PF_FpLong alpha) {
  PF_FpLong ret = 0;
...
static PF_Err ColorSet8(refconType refcon, A_long xL, A_long yL, PF_Pixel8 *inP,
                        PF_Pixel8 *outP) {
  PF_Err err = PF_Err_NONE;
  ParamInfo8 *niP = reinterpret_cast<ParamInfo8 *>(refcon);
...
static PF_Err debug8(CFsAE *ae, ParamInfo8 *pi) {
  PF_Err err = PF_Err_NONE;
  ...
  if (pi.TargetCount > 0) {
    ...
    if ((pi.info.hi_length > 0) || (pi.info.sw_length > 0)) {
      ...
      if ((pi.info.hi_blur > 0) || (pi.info.sw_blur > 0)) {
        pi.scanlineH = ae->NewHandle(pi.scanlineSize * sizeof(A_u_char) * 4 +
