...
static PF_Err GetParams(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (!err) {
    ...
    if (infoP->rect.left > infoP->rect.right)
      swapLong(&infoP->rect.left, &infoP->rect.right);
    if (infoP->rect.top > infoP->rect.bottom)
      swapLong(&infoP->rect.top, &infoP->rect.bottom);
  }
...
static PF_Err Exec(CFsAE *ae, ParamInfo *infoP) {
  ...
  if (infoP->value > 0) {
    CFsBuffer buf =
        ae->NewBuffer(SPD_RAND_MAX * sizeof(A_u_char));  // + SPD_RAND_MAX
    if (buf.alive() == FALSE) {
